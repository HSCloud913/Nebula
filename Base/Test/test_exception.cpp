...
TEST(ExceptionTest, CreateExceptionWithFormattingError)
{
...
TEST(ExceptionTest, NoCopy)
{
	...
	// ne::Exception exCopy = ex; // 이 부분이 컴파일 오류를 발생시킴
}

// 예외가 이동될 수 있는지 테스트
TEST(ExceptionTest, MoveConstructible)
{
	const std::string module = "TestModule";
	const std::string message = "An error occurred";

	ne::Exception ex(module, message);
	ne::Exception exMoved = std::move(ex); // 이동 생성자 테스트
...
TEST(ExceptionTest, MoveAssignable)
{
	...
	ne::Exception ex1(module1, message1);
	ne::Exception ex2(module2, message2);
