...
#include "Network/Protocol/Http/Http1/Client.h"
...
ne::Task<ne::Result<HttpResponse, ne::OsError>> Client::Execute(string_view_t _host, uint16_t _port, HttpRequest _request) const
{
	...
	const ne::io::BufferView sendView{ nullptr, const_cast<byte_t*>(reinterpret_cast<const byte_t*>(reqStr.data())), reqStr.size() };

	if (auto r = co_await stream.Send(sendView); r.IsError())
	{
		auto err = std::move(r.Error());
		err.Context("[Http1Client/Execute]");
		co_return ne::Result<HttpResponse, ne::OsError>::Error(std::move(err));
...
{
	static constexpr string_view_t Methods[] = { "GET", "POST", "PUT", "DELETE", "PATCH", "HEAD", "OPTIONS" };
...
ne::Result<HttpResponse, ne::OsError> Client::ParseResponse(string_view_t _raw)
{
	...
	if (sep == string_view_t::npos)
		return ne::Result<HttpResponse, ne::OsError>::Error(ne::OsError{ 0, "incomplete HTTP response" });
	...
	const auto codeView = (sp2 != string_view_t::npos) ? statusLine.substr(sp1 + 1, sp2 - sp1 - 1) : statusLine.substr(sp1 + 1);
