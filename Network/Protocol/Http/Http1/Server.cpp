...
void Server::Stop() noexcept { running = false; }
...
ne::Task<void> Server::HandleConnection(Socket _sock)
{
	...
	const ne::io::BufferView sendView{ nullptr, const_cast<byte_t*>(reinterpret_cast<const byte_t*>(respStr.data())), respStr.size() };

	(void)co_await stream.Send(sendView);
	(void)stream.Close();
}
...
ne::Result<HttpRequest, ne::OsError> Server::ParseRequest(const string_view_t _raw)
{
	...
	req.path = string_t((sp2 != string_view_t::npos) ? requestLine.substr(sp1 + 1, sp2 - sp1 - 1) : requestLine.substr(sp1 + 1));
...
std::size_t Server::ParseContentLength(const string_view_t _headers) noexcept
{
	...
	const auto numStr = (end != string_view_t::npos) ? _headers.substr(start, end - start) : _headers.substr(start);
...
string_t Server::SerializeResponse(const HttpResponse& _response)
{
	...
	s += ' ';
	s += _response.statusText.empty() ? string_t(DefaultStatusText(_response.status)) : _response.statusText;
	s += "\r\n";
