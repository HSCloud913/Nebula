...
Http2Server::Http2Server(std::unique_ptr<ne::network::IStream> _stream, Http2Handler _handler) noexcept
	: stream(std::move(_stream))
	, handler(std::move(_handler)) {}

Http2Server::~Http2Server() { if (stream) (void)stream->Close(); }
...
ne::Task<ne::Result<void, ne::HttpError>> Http2Server::Run()
{
	...
	{
		auto r = co_await stream->Receive(ne::io::BufferView{ nullptr, prefaceBuf.data() + read, kClientPreface.size() - read });
		if (r.IsError())
			co_return ne::Result<void, ne::HttpError>::Error(ne::HttpError{ r.Error().What() }.Context("[Http2Server/Run/ReadPreface]"));
		if (r.Value() == 0) co_return ne::Result<void, ne::HttpError>::Error(ne::HttpError{ "connection closed" });
		read += r.Value();
	}
	...
	for (std::size_t i = 0; i < kClientPreface.size(); ++i)
	{
		if (prefaceBuf[i] != static_cast<ne::byte_t>(kClientPreface[i]))
			co_return ne::Result<void, ne::HttpError>::Error(ne::HttpError{ "invalid client preface" }.Context("[Http2Server/Run]"));
	}
	...
	while (true)
	{
		...
		{
			auto r = co_await stream->Receive(ne::io::BufferView{ nullptr, hbuf.data() + hr, kFrameHeaderSize - hr });
			if (r.IsError())
				co_return ne::Result<void, ne::HttpError>::Error(ne::HttpError{ r.Error().What() }.Context("[Http2Server/Run/FrameHeader]"));
		...
		while (pr < fh.length)
		{
			auto r = co_await stream->Receive(ne::io::BufferView{ nullptr, payload.data() + pr, fh.length - pr });
			if (r.IsError())
				co_return ne::Result<void, ne::HttpError>::Error(ne::HttpError{ r.Error().What() }.Context("[Http2Server/Run/Payload]"));
			pr += r.Value();
		}
...
ne::Task<ne::Result<void, ne::HttpError>> Http2Server::SendPreface() { co_return ne::Result<void, ne::HttpError>::Ok(); }
...
ne::Task<ne::Result<void, ne::HttpError>> Http2Server::SendHeaders(uint32_t _streamId, const ne::network::HttpResponse& _resp)
{
	const bool_t hasBody = !_resp.body.empty();
	const auto block = Hpack::Encode("", "", "", "", { { ":status", std::format("{}", _resp.status) } });
	...
	if (r.IsError())
		co_return ne::Result<void, ne::HttpError>::Error(ne::HttpError{ r.Error().What() }.Context("[Http2Server/SendHeaders]"));

	if (!block.empty())
	{
		auto r2 = co_await stream->Send(ne::io::BufferView{ nullptr, const_cast<ne::byte_t*>(block.data()), block.size() });
		if (r2.IsError())
			co_return ne::Result<void, ne::HttpError>::Error(ne::HttpError{ r2.Error().What() }.Context("[Http2Server/SendHeaders/Block]"));
	}

	co_return ne::Result<void, ne::HttpError>::Ok();
}

ne::Task<ne::Result<void, ne::HttpError>> Http2Server::SendData(uint32_t _streamId, const ne::string_t& _body)
{
	...
	auto r = co_await stream->Send(ne::io::BufferView{ nullptr, hdr.data(), hdr.size() });
	if (r.IsError())
		co_return ne::Result<void, ne::HttpError>::Error(ne::HttpError{ r.Error().What() }.Context("[Http2Server/SendData/Header]"));

	auto r2 = co_await stream->Send(ne::io::BufferView{ nullptr, const_cast<ne::byte_t*>(body), _body.size() });
	if (r2.IsError())
		co_return ne::Result<void, ne::HttpError>::Error(ne::HttpError{ r2.Error().What() }.Context("[Http2Server/SendData/Payload]"));
