...
[[nodiscard]] inline uint32_t ReadU24(const ne::byte_t* _p) noexcept { return (uint32_t(_p[0]) << 16) | (uint32_t(_p[1]) << 8) | uint32_t(_p[2]); }
[[nodiscard]] inline uint32_t ReadU32(const ne::byte_t* _p) noexcept { return (uint32_t(_p[0]) << 24) | (uint32_t(_p[1]) << 16) | (uint32_t(_p[2]) << 8) | uint32_t(_p[3]); }
[[nodiscard]] inline uint16_t ReadU16(const ne::byte_t* _p) noexcept { return uint16_t((uint16_t(_p[0]) << 8) | uint16_t(_p[1])); }
...
inline void BuildFrameHeader(ne::byte_t* _buf, uint32_t _length, FrameType _type, uint8_t _flags, uint32_t _streamId) noexcept
{
...
struct FrameHeader
{
	...
	[[nodiscard]] static FrameHeader Parse(const ne::byte_t* _buf) noexcept { return { ReadU24(_buf), FrameType(_buf[3]), _buf[4], ReadU32(_buf + 5) & 0x7FFFFFFFu }; }
};
