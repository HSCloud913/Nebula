...
struct StaticEntry { ne::string_view_t name, value; };
static constexpr StaticEntry kStatic[] = {
	{":authority",                  ""},               // 1
	{":method",                     "GET"},            // 2
	{":method",                     "POST"},           // 3
	{":path",                       "/"},              // 4
...
// ── Integer helpers ───────────────────────────────────────────────────────────
static void EncodeInt(std::vector<ne::byte_t>& _out,
                      uint32_t _val, uint8_t _prefixBits, uint8_t _firstByte)
{
...
static bool ParseInt(const ne::byte_t* _data, std::size_t _len,
                     std::size_t& _i, uint8_t _prefixBits, uint32_t& _out)
{
...
static bool ParseStr(const ne::byte_t* _data, std::size_t _len,
                     std::size_t& _i, ne::string_t& _out)
{
	if (_i >= _len) return false;
	...
	if (huffman)
	{
		if (!HuffDecode(_data + _i, strLen, _out)) _out.clear();
	}
	else
	{
		_out.assign(reinterpret_cast<const char*>(_data + _i), strLen);
	}
	_i += strLen;
	return true;
}
...
namespace Hpack
{
		...
		const ne::network::HttpHeaders& _headers)
	{
		...
		else { out.push_back(0x40u); EncodeStr(out, ":method"); EncodeStr(out, _method); }

		// :path — indexed if "/" (4)
		if (_path == "/") out.push_back(0x84u);
		else { out.push_back(0x44u); EncodeStr(out, _path); } // name index 4

		// :scheme — indexed http(6) / https(7)
		if      (_scheme == "http")  out.push_back(0x86u);
		else if (_scheme == "https") out.push_back(0x87u);
		else { out.push_back(0x40u); EncodeStr(out, ":scheme"); EncodeStr(out, _scheme); }
