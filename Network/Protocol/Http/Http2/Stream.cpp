...
ne::Task<ne::Result<void, ne::HttpError>> Http2Stream::SendHeaders(const ne::network::HttpHeaders& _headers, bool_t _endStream, const std::vector<ne::byte_t>& _hpackBlock)
{
	(void)_headers;
	...
	if (r.IsError())
		co_return ne::Result<void, ne::HttpError>::Error(ne::HttpError{ r.Error().What() }.Context("[Http2Stream/SendHeaders/Header]"));
...
ne::Task<ne::Result<void, ne::HttpError>> Http2Stream::SendData(std::span<const ne::byte_t> _data, bool_t _endStream)
{
	std::array<ne::byte_t, kFrameHeaderSize> header{};
	...
	auto r = co_await transport->Send(ne::io::BufferView{ nullptr, header.data(), header.size() });
	if (r.IsError())
		co_return ne::Result<void, ne::HttpError>::Error(ne::HttpError{ r.Error().What() }.Context("[Http2Stream/SendData/Header]"));
...
ne::Task<ne::Result<ne::network::HttpResponse, ne::HttpError>> Http2Stream::ReceiveResponse(const ne::network::HttpHeaders& _responseHeaders)
{
	...
	while (state != StreamState::HalfClosedRemote && state != StreamState::Closed)
	{
		...
		{
			auto r = co_await transport->Receive(ne::io::BufferView{ nullptr, headerBuf.data() + total, kFrameHeaderSize - total });
			if (r.IsError())
				co_return ne::Result<ne::network::HttpResponse, ne::HttpError>::Error(ne::HttpError{ r.Error().What() }.Context("[Http2Stream/ReceiveResponse/FrameHeader]"));
			if (r.Value() == 0)
				co_return ne::Result<ne::network::HttpResponse, ne::HttpError>::Error(ne::HttpError{ "connection closed" }.Context("[Http2Stream/ReceiveResponse]"));
			total += r.Value();
		}
		...
		if (fh.type == FrameType::Data)
		{
			...
			{
				auto r = co_await transport->Receive(ne::io::BufferView{ nullptr, payload.data() + read, fh.length - read });
				if (r.IsError())
					co_return ne::Result<ne::network::HttpResponse, ne::HttpError>::Error(ne::HttpError{ r.Error().What() }.Context("[Http2Stream/ReceiveResponse/Data]"));
				read += r.Value();
			}
		...
		else
		{
			...
			{
				auto r = co_await transport->Receive(ne::io::BufferView{ nullptr, discard.data() + dread, fh.length - dread });
				if (r.IsError()) break;
				dread += r.Value();
			}
			if (fh.flags & Flag::EndStream)
