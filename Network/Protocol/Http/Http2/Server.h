...
using Http2Handler = std::function<ne::Task<ne::network::HttpResponse>(ne::network::HttpRequest)>;
...
private:
	...
	[[nodiscard]] ne::Task<ne::Result<void, ne::HttpError>> SendHeaders(uint32_t _streamId, const ne::network::HttpResponse& _resp);
	[[nodiscard]] ne::Task<ne::Result<void, ne::HttpError>> SendData(uint32_t _streamId, const ne::string_t& _body);
};
