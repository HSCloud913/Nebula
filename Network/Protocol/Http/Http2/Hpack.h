...
namespace Hpack
{
	[[nodiscard]] std::vector<ne::byte_t> Encode(ne::string_view_t _method, ne::string_view_t _path, ne::string_view_t _scheme, ne::string_view_t _authority, const ne::network::HttpHeaders& _headers);

	// Returns headers including pseudo-headers (:status, :path …)
	[[nodiscard]] ne::network::HttpHeaders Decode(const ne::byte_t* _data, std::size_t _len);
}
