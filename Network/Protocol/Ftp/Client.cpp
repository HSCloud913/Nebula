...
Client::Client(std::unique_ptr<ne::network::IStream> _control, DataStreamFactory_t _dataFactory) noexcept
	: controlStream(std::move(_control))
	, dataFactory(std::move(_dataFactory)) {}

Client::~Client() { if (controlStream) { (void)controlStream->Close(); } }
...
ne::Task<ne::Result<Client, ne::Error>> Client::Connect(std::unique_ptr<ne::network::IStream> _control, DataStreamFactory_t _dataFactory, const FtpConfig& _config)
{
	...
	if (result.IsError()) co_return ne::Result < Client, ne::Error > ::Error(std::move(result.Error()));
	if (result.Value().code != 220)
		co_return ne::Result < Client, ne::Error > ::Error(ne::Error(std::format("unexpected banner: {}", result.Value().code)).Context("[FtpClient/Connect]"));

	if (auto loginResult = co_await client.Login(_config.username, _config.password); loginResult.IsError()) co_return ne::Result < Client, ne::Error > ::Error(std::move(loginResult.Error()));
...
ne::Task<ne::Result<FtpReply, ne::Error>> Client::ReadReply()
{
	...
	auto readLine = [this]() -> ne::Task<ne::Result<string_t, ne::Error>>
	{
		while (true)
		{
			...
			if (result.IsError())
				co_return ne::Result<string_t, ne::Error>::Error(ne::Error(result.Error().What()).Context("[FtpClient/ReadReply]"));

			if (result.Value() == 0)
				co_return ne::Result<string_t, ne::Error>::Error(ne::Error("connection closed").Context("[FtpClient/ReadReply]"));
	...
	if (result.IsError()) co_return ne::Result<FtpReply, ne::Error>::Error(std::move(result.Error()));

	const string_t& first = result.Value();
	if (first.size() < 3) co_return ne::Result<FtpReply, ne::Error>::Error(ne::Error("malformed FTP reply"));

	int code = 0;
	for (int i = 0; i < 3; ++i)
	{
		if (first[i] < '0' || first[i] > '9') co_return ne::Result<FtpReply, ne::Error>::Error(ne::Error("malformed FTP reply code"));

		code = code * 10 + (first[i] - '0');
	}

	// Single-line: 4th char is space or string ends at 3
	if (first.size() < 4 || first[3] != '-') { co_return ne::Result<FtpReply, ne::Error>::Ok(FtpReply{ code, first.size() > 4 ? first.substr(4) : string_t{} }); }
	...
	while (true)
	{
		...
		if (line.size() >= 4 && line.substr(0, 3) == codeStr && line[3] == ' ')
			co_return ne::Result<FtpReply, ne::Error>::Ok(FtpReply{ code, line.size() > 4 ? line.substr(4) : string_t{} });
	}
}
...
ne::Task<ne::Result<FtpReply, ne::Error>> Client::SendCommand(string_view_t _cmd)
{
	...
	if (result.IsError())
		co_return ne::Result<FtpReply, ne::Error>::Error(ne::Error(result.Error().What()).Context("[FtpClient/SendCommand]"));
...
ne::Task<ne::Result<std::pair<string_t, uint16_t>, ne::Error>> Client::Pasv()
{
	...
	if (r.Value().code != 227)
		co_return ne::Result<std::pair<string_t, uint16_t>, ne::Error>::Error(ne::Error(std::format("PASV failed: {}", r.Value().message)).Context("[FtpClient/Pasv]"));

	auto parsed = ParsePassiveReply(r.Value().message);
	if (!parsed)
		co_return ne::Result<std::pair<string_t, uint16_t>, ne::Error>::Error(ne::Error("failed to parse PASV response").Context("[FtpClient/Pasv]"));
...
ne::Task<ne::Result<void, ne::Error>> Client::Login(string_view_t _user, string_view_t _password)
{
	...
	if (ur.Value().code != 331)
		co_return ne::Result<void, ne::Error>::Error(ne::Error(std::format("USER failed: {}", ur.Value().message)).Context("[FtpClient/Login]"));
	...
	if (pr.Value().code != 230)
		co_return ne::Result<void, ne::Error>::Error(ne::Error(std::format("PASS failed: {}", pr.Value().message)).Context("[FtpClient/Login]"));
...
ne::Task<ne::Result<void, ne::Error>> Client::Quit()
{
	...
	if (controlStream) (void)controlStream->Close();
...
ne::Task<ne::Result<void, ne::Error>> Client::SetTransferType(TransferType _type)
{
	...
	if (result.Value().code != 200)
		co_return ne::Result<void, ne::Error>::Error(ne::Error(std::format("TYPE failed: {}", result.Value().message)).Context("[FtpClient/SetTransferType]"));
...
ne::Task<ne::Result<string_t, ne::Error>> Client::Pwd()
{
	...
	if (result.IsError()) co_return ne::Result<string_t, ne::Error>::Error(std::move(result.Error()));

	if (result.Value().code != 257)
		co_return ne::Result<string_t, ne::Error>::Error(ne::Error(std::format("PWD failed: {}", result.Value().message)).Context("[FtpClient/Pwd]"));

	const auto& message = result.Value().message;
	const auto q1 = message.find('"');
	const auto q2 = message.rfind('"');
	if (q1 == string_t::npos || q1 == q2) co_return ne::Result<string_t, ne::Error>::Ok(message);
...
ne::Task<ne::Result<void, ne::Error>> Client::Cwd(string_view_t _path)
{
	...
	if (result.Value().code != 250)
		co_return ne::Result<void, ne::Error>::Error(ne::Error(std::format("CWD failed: {}", result.Value().message)).Context("[FtpClient/Cwd]"));
...
ne::Task<ne::Result<void, ne::Error>> Client::Mkd(string_view_t _path)
{
	...
	if (result.Value().code != 257)
		co_return ne::Result<void, ne::Error>::Error(ne::Error(std::format("MKD failed: {}", result.Value().message)).Context("[FtpClient/Mkd]"));
...
ne::Task<ne::Result<void, ne::Error>> Client::Rmd(string_view_t _path)
{
	...
	if (result.Value().code != 250)
		co_return ne::Result<void, ne::Error>::Error(ne::Error(std::format("RMD failed: {}", result.Value().message)).Context("[FtpClient/Rmd]"));
...
ne::Task<ne::Result<void, ne::Error>> Client::Dele(string_view_t _path)
{
	...
	if (result.Value().code != 250)
		co_return ne::Result<void, ne::Error>::Error(ne::Error(std::format("DELE failed: {}", result.Value().message)).Context("[FtpClient/Dele]"));
...
ne::Task<ne::Result<void, ne::Error>> Client::Rename(string_view_t _from, string_view_t _to)
{
	...
	if (result.Value().code != 350)
		co_return ne::Result<void, ne::Error>::Error(ne::Error(std::format("RNFR failed: {}", result.Value().message)).Context("[FtpClient/Rename]"));

	result = co_await SendCommand("RNTO", _to);
	if (result.IsError()) co_return ne::Result<void, ne::Error>::Error(std::move(result.Error()));

	if (result.Value().code != 250)
		co_return ne::Result<void, ne::Error>::Error(ne::Error(std::format("RNTO failed: {}", result.Value().message)).Context("[FtpClient/Rename]"));
...
ne::Task<ne::Result<std::vector<FtpEntry>, ne::Error>> Client::List(string_view_t _path)
{
	...
	if (dataR.IsError())
		co_return ne::Result<std::vector<FtpEntry>, ne::Error>::Error(ne::Error(dataR.Error().What()).Context("[FtpClient/List/connect]"));
	auto dataStream = std::move(dataR.Value());

	// Try MLSD (RFC 3659); fall back to LIST on 500/502
	auto cmdR = _path.empty() ? co_await SendCommand("MLSD") : co_await SendCommand("MLSD", _path);
	...
	if (cmdR.Value().code == 500 || cmdR.Value().code == 502)
	{
		...
		if (dr2.IsError())
			co_return ne::Result<std::vector<FtpEntry>, ne::Error>::Error(ne::Error(dr2.Error().What()).Context("[FtpClient/List/fallback/connect]"));
		dataStream = std::move(dr2.Value());

		auto listR = _path.empty() ? co_await SendCommand("LIST") : co_await SendCommand("LIST", _path);
		...
		if (listR.Value().code != 125 && listR.Value().code != 150)
			co_return ne::Result<std::vector<FtpEntry>, ne::Error>::Error(ne::Error(std::format("LIST failed: {}", listR.Value().message)).Context("[FtpClient/List]"));
	}
	else if (cmdR.Value().code != 125 && cmdR.Value().code != 150) { co_return ne::Result<std::vector<FtpEntry>, ne::Error>::Error(ne::Error(std::format("MLSD failed: {}", cmdR.Value().message)).Context("[FtpClient/List]")); }
...
ne::Task<ne::Result<void, ne::Error>> Client::Get(string_view_t _remote, const SinkFn_t& _sink, uint64_t _offset)
{
	...
	auto dataR = co_await dataFactory(dHost, dPort);
	if (dataR.IsError())
		co_return ne::Result<void, ne::Error>::Error(ne::Error(dataR.Error().What()).Context("[FtpClient/Get/connect]"));
	auto dataStream = std::move(dataR.Value());
	...
	if (_offset > 0)
	{
		...
		if (restR.Value().code != 350)
			co_return ne::Result<void, ne::Error>::Error(ne::Error(std::format("REST failed: {}", restR.Value().message)).Context("[FtpClient/Get]"));
	}
	...
	if (retrR.Value().code != 125 && retrR.Value().code != 150)
		co_return ne::Result<void, ne::Error>::Error(ne::Error(std::format("RETR failed: {}", retrR.Value().message)).Context("[FtpClient/Get]"));
...
ne::Task<ne::Result<void, ne::Error>> Client::Put(string_view_t _remote, std::span<const ne::byte_t> _data, uint64_t _offset)
{
	...
	auto dataR = co_await dataFactory(dHost, dPort);
	if (dataR.IsError())
		co_return ne::Result<void, ne::Error>::Error(ne::Error(dataR.Error().What()).Context("[FtpClient/Put/connect]"));
	auto dataStream = std::move(dataR.Value());
	...
	if (_offset > 0)
	{
		...
		if (restR.IsError()) co_return ne::Result<void, ne::Error>::Error(std::move(restR.Error()));
		if (restR.Value().code != 350)
			co_return ne::Result<void, ne::Error>::Error(ne::Error(std::format("REST failed: {}", restR.Value().message)).Context("[FtpClient/Put]"));
	}
	...
	if (storR.Value().code != 125 && storR.Value().code != 150)
		co_return ne::Result<void, ne::Error>::Error(ne::Error(std::format("STOR failed: {}", storR.Value().message)).Context("[FtpClient/Put]"));
	...
	while (sent < _data.size())
	{
		...
		if (r.IsError())
			co_return ne::Result<void, ne::Error>::Error(ne::Error(r.Error().What()).Context("[FtpClient/Put/send]"));
		sent += r.Value();
	}
	(void)dataStream->Close();
	(void)co_await ReadReply(); // 226
