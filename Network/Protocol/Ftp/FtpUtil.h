...
[[nodiscard]] inline std::optional<std::pair<string_t, uint16_t>> ParsePassiveReply(string_view_t _reply)
{
	const auto lp = _reply.find('(');
	const auto rp = _reply.find(')');
	if (lp == string_view_t::npos || rp == string_view_t::npos || rp <= lp) return std::nullopt;
	...
	string_t host = std::to_string(parts[0]) + '.' + std::to_string(parts[1]) + '.' + std::to_string(parts[2]) + '.' + std::to_string(parts[3]);
...
[[nodiscard]] inline std::optional<FtpEntry> ParseMlsdLine(string_view_t _line)
{
	...
	while (pos < facts.size())
	{
		...
		if (eq != string_view_t::npos)
		{
			...
			if (key == "type") { entry.isDirectory = (val == "dir" || val == "cdir" || val == "pdir"); }
			else if (key == "size")
			{
				uint64_t sz = 0;
