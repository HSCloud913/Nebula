...
namespace ne::network
{
	...
	ne::Task<ne::io::IoResult<PlainStream>> PlainStream::Connect(const string_view_t _host, const uint16_t _port, ne::io::Context& _context, std::stop_token _stopToken, ne::memory::IAllocator* _allocator)
	{
		...
		auto resolved = co_await dns::Resolve(_host, _context);
	...
	ne::Task<ne::io::IoResult<std::size_t>> PlainStream::Receive(const ne::memory::BufferView _data, std::stop_token _stopToken)
	{
	...
	ne::Task<ne::io::IoResult<std::size_t>> PlainStream::Receivev(const ne::memory::BufferChain& _chain, std::stop_token _stopToken)
	{
		using R = ne::io::IoResult<std::size_t>;
	...
	ne::Task<ne::io::IoResult<std::size_t>> PlainStream::Send(const ne::memory::BufferView _data, std::stop_token _stopToken)
	{
		using R = ne::io::IoResult<std::size_t>;
	...
	ne::Task<ne::io::IoResult<std::size_t>> PlainStream::Sendv(const ne::memory::BufferChain& _chain, std::stop_token _stopToken)
	{
		using R = ne::io::IoResult<std::size_t>;
	...
	ne::io::IoResult<void_t> PlainStream::Close()
	{
		using R = ne::io::IoResult<void_t>;
	...
	ne::Task<ne::io::IoResult<std::size_t>> PlainStream::SendFile(const ne::io::file_t _file, const ulonglong_t _offset, const std::size_t _length, const ne::memory::BufferChain& _head, const ne::memory::BufferChain& _tail, std::stop_token _stopToken)
	{
		using R = ne::io::IoResult<std::size_t>;
	...
	ne::Task<ne::io::IoResult<std::size_t>> PlainStream::ReceiveFile(ne::io::File& _file, const ulonglong_t _offset, const std::size_t _length, std::stop_token _stopToken)
	{
		...
		while (remaining > 0)
		{
			...
			auto received = co_await Receive(ne::memory::BufferView{ chunk, want }, _stopToken);
	...
	ne::Task<ne::io::IoResult<std::size_t>> PlainStream::SendRegistered(const ne::io::RegisteredBuffer& _buffer, std::stop_token _stopToken)
	{
		...
		const ne::memory::BufferView view = _buffer.View();
