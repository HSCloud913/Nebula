...
} static ne::Task<ne::Result<void, ne::OsError>> WaitSocket(const socket_t _fd, ne::io::IIoEngine& _engine, LIBSSH2_SESSION* _session)
{
	...
	if (dir & LIBSSH2_SESSION_BLOCK_INBOUND) { if (auto result = co_await ne::io::ReceiveAwaitable{ _fd, _engine }; result.IsError()) co_return ne::Result<void, ne::OsError>::Error(std::move(result.Error())); }
	if (dir & LIBSSH2_SESSION_BLOCK_OUTBOUND) { if (auto result = co_await ne::io::SendAwaitable{ _fd, _engine }; result.IsError()) co_return ne::Result<void, ne::OsError>::Error(std::move(result.Error())); }
	...
	, session(_session)
	, channel(_channel) {} SshStream::SshStream(SshStream&& _other) noexcept
	: transport(std::move(_other.transport))
	, sshConfig(std::move(_other.sshConfig))
	, session(std::exchange(_other.session, nullptr))
	, channel(std::exchange(_other.channel, nullptr)) {} SshStream& SshStream::operator=(SshStream&& _other) noexcept
{
	if (this != &_other)
	{
		(void)Close();
...
} ne::Task<ne::Result<SshStream, ne::OsError>> SshStream::Connect(Socket&& _socket, ne::io::IIoEngine& _engine, const SshConfig& _config, ne::memory::IAllocator* _allocator)
{
	...
	if (!tempSession)
		co_return ne::Result < SshStream, ne::OsError > ::Error(ne::OsError{ 0, "libssh2_session_init failed" });
...
} ne::Task<ne::Result<void, ne::OsError>> SshStream::Handshake()
{
	...
	while (true)
	{
		...
		if (sshResult == LIBSSH2_ERROR_EAGAIN) { if (auto result = co_await WaitSocket(transport.Handle(), transport.Engine(), nativeSession); result.IsError()) co_return ne::Result<void, ne::OsError>::Error(std::move(result.Error())); }
		else { co_return ne::Result<void, ne::OsError>::Error(Ssh2Error(nativeSession, "[SshStream/Handshake/SessionHandshake]")); }
	}
	...
	if (!sshConfig.privateKeyFile.empty())
	{
		while (true)
		{
			...
			if (sshResult == LIBSSH2_ERROR_EAGAIN) { if (auto result = co_await WaitSocket(transport.Handle(), transport.Engine(), nativeSession); result.IsError()) co_return ne::Result<void, ne::OsError>::Error(std::move(result.Error())); }
			else { co_return ne::Result<void, ne::OsError>::Error(Ssh2Error(nativeSession, "[SshStream/Handshake/PubkeyAuth]")); }
		}
	}
	else if (!sshConfig.password.empty())
	{
		while (true)
		{
			...
			if (sshResult == LIBSSH2_ERROR_EAGAIN) { if (auto result = co_await WaitSocket(transport.Handle(), transport.Engine(), nativeSession); result.IsError()) co_return ne::Result<void, ne::OsError>::Error(std::move(result.Error())); }
			else { co_return ne::Result<void, ne::OsError>::Error(Ssh2Error(nativeSession, "[SshStream/Handshake/PasswordAuth]")); }
		}
	}
	...
	while (true)
	{
		...
		if (libssh2_session_last_errno(nativeSession) == LIBSSH2_ERROR_EAGAIN) { if (auto result = co_await WaitSocket(transport.Handle(), transport.Engine(), nativeSession); result.IsError()) co_return ne::Result<void, ne::OsError>::Error(std::move(result.Error())); }
		else { co_return ne::Result<void, ne::OsError>::Error(Ssh2Error(nativeSession, "[SshStream/Handshake/OpenChannel]")); }
	}
...
} ne::Task<ne::Result<std::size_t, ne::OsError>> SshStream::Send(ne::io::BufferView _data)
{
	...
	while (totalSent < dataSpan.size())
	{
		...
		if (bytes >= 0) { totalSent += static_cast<std::size_t>(bytes); }
		else if (bytes == LIBSSH2_ERROR_EAGAIN) { if (auto result = co_await WaitSocket(transport.Handle(), transport.Engine(), nativeSession); result.IsError()) co_return ne::Result<std::size_t, ne::OsError>::Error(std::move(result.Error())); }
		else { co_return ne::Result<std::size_t, ne::OsError>::Error(Ssh2Error(nativeSession, "[SshStream/Send]")); }
	}
...
} ne::Task<ne::Result<std::size_t, ne::OsError>> SshStream::Receive(ne::io::BufferView _data)
{
	...
	while (true)
	{
		...
		if (bytes == 0 || libssh2_channel_eof(nativeChannel)) { co_return ne::Result<std::size_t, ne::OsError>::Ok(0); }

		if (bytes == LIBSSH2_ERROR_EAGAIN) { if (auto result = co_await WaitSocket(transport.Handle(), transport.Engine(), nativeSession); result.IsError()) co_return ne::Result<std::size_t, ne::OsError>::Error(std::move(result.Error())); }
		else { co_return ne::Result<std::size_t, ne::OsError>::Error(Ssh2Error(nativeSession, "[SshStream/Receive]")); }
	}
...
SshStream::SshStream(PlainStream&& _transport, void*, void*) noexcept
	: transport(std::move(_transport)) {}
...
ne::Task<ne::Result<SshStream, ne::OsError>> SshStream::Connect(Socket&&, ne::io::IIoEngine&, const SshConfig&, ne::memory::IAllocator*) { co_return ne::Result < SshStream, ne::OsError > ::Error(NoLibssh2("[SshStream/Connect]")); }

ne::Task<ne::Result<void, ne::OsError>> SshStream::Handshake() { co_return ne::Result<void, ne::OsError>::Error(NoLibssh2("[SshStream/Handshake]")); }

ne::Task<ne::Result<std::size_t, ne::OsError>> SshStream::Send(ne::io::BufferView) { co_return ne::Result<std::size_t, ne::OsError>::Error(NoLibssh2("[SshStream/Send]")); }

ne::Task<ne::Result<std::size_t, ne::OsError>> SshStream::Receive(ne::io::BufferView) { co_return ne::Result<std::size_t, ne::OsError>::Error(NoLibssh2("[SshStream/Receive]")); }

ne::Task<ne::Result<std::size_t, ne::OsError>> SshStream::Sendv(const ne::io::BufferChain&) { co_return ne::Result<std::size_t, ne::OsError>::Error(NoLibssh2("[SshStream/Sendv]")); }

ne::Task<ne::Result<void, ne::OsError>> SshStream::Shutdown() { co_return ne::Result<void, ne::OsError>::Ok(); }

ne::Result<void, ne::OsError> SshStream::Close() { return ne::Result<void, ne::OsError>::Ok(); }
