...
static ne::OsError SchannelError(SECURITY_STATUS _ss, string_view_t _ctx)
{
	...
	err.Context(_ctx);
	return err;
} TlsStream::TlsStream(PlainStream&& _transport, void* _credHandle, void* _ctxHandle, void* _messageBuffer) noexcept
	: transport(std::move(_transport))
	, credHandle(_credHandle)
	, ctxHandle(_ctxHandle)
	, messageBuffer(_messageBuffer) {} TlsStream::TlsStream(TlsStream&& _other) noexcept
	...
	, messageBuffer(std::exchange(_other.messageBuffer, nullptr)) {} TlsStream& TlsStream::operator=(TlsStream&& _other) noexcept
{
	if (this != &_other)
	{
		(void)Close();
...
} ne::Task<ne::Result<TlsStream, ne::OsError>> TlsStream::Connect(Socket&& _socket, ne::io::IIoEngine& _engine, string_view_t _host, const TlsConfig& _config, ne::memory::IAllocator* _allocator)
{
	...
	if (!functionTable)
		co_return ne::Result < TlsStream, ne::OsError > ::Error(ne::OsError{ 0, "SChannel: secur32.dll load failed" });
	...
	if (ss != SEC_E_OK)
		co_return ne::Result < TlsStream, ne::OsError > ::Error(SchannelError(ss, "[TlsStream/Connect/AcquireCred]"));

	auto plainStream = PlainStream::Create(std::move(_socket), _engine, _allocator);
	if (plainStream.IsError()) co_return ne::Result < TlsStream, ne::OsError > ::Error(std::move(plainStream.Error()).Context("[TlsStream/Connect]"));

	TlsStream stream(std::move(plainStream.Value()), tempCredHandle.release(), nullptr, std::unique_ptr<TlsMessageBuffer>(new TlsMessageBuffer(TlsMessageBuffer::Allocate())).release());
	stream.sniHost = string_t(_host);

	if (auto result = co_await stream.Handshake(); result.IsError()) co_return ne::Result < TlsStream, ne::OsError > ::Error(std::move(result.Error()));
...
} ne::Task<ne::Result<TlsStream, ne::OsError>> TlsStream::Accept(Socket&& _socket, ne::io::IIoEngine& _engine, const TlsConfig& _config, ne::memory::IAllocator* _allocator)
{
	...
	if (!pfxFile)
		co_return ne::Result < TlsStream, ne::OsError > ::Error(ne::OsError{ 0, "SChannel: failed to open PFX file" });
	...
	if (!certStore)
		co_return ne::Result < TlsStream, ne::OsError > ::Error(ne::OsError{ ::GetLastError(), "SChannel: PFXImportCertStore failed" });
	...
	if (!certCtx)
	{
		...
		co_return ne::Result < TlsStream, ne::OsError > ::Error(ne::OsError{ ::GetLastError(), "SChannel: no cert with private key in PFX" });
	}
	...
	if (ss != SEC_E_OK)
		co_return ne::Result < TlsStream, ne::OsError > ::Error(SchannelError(ss, "[TlsStream/Accept/AcquireCred]"));

	auto plainStream = PlainStream::Create(std::move(_socket), _engine, _allocator);
	if (plainStream.IsError()) co_return ne::Result < TlsStream, ne::OsError > ::Error(std::move(plainStream.Error()).Context("[TlsStream/Accept]"));
	...
	while (true)
	{
		...
		if (auto result = co_await ne::io::ReceiveAwaitable{ stream.transport.Handle(), _engine }; result.IsError()) co_return ne::Result < TlsStream, ne::OsError > ::Error(std::move(result.Error()));

		const int received = ::recv(stream.transport.Handle(), reinterpret_cast<char*>(span.data() + dataInBuffer), static_cast<int>(span.size() - dataInBuffer), 0);
		if (received <= 0)
			co_return ne::Result < TlsStream, ne::OsError > ::Error(ne::OsError{ ne::LastOsError() }.Context("[TlsStream/Accept/Recv]"));
		...
		ULONG retFlags = 0;
		ss = functionTable->AcceptSecurityContext(nativeCredHandle, isFirstCall ? nullptr : tempCtxHandle.get(), &inDesc, ASC_REQ_SEQUENCE_DETECT | ASC_REQ_REPLAY_DETECT | ASC_REQ_CONFIDENTIALITY | ASC_REQ_EXTENDED_ERROR | ASC_REQ_ALLOCATE_MEMORY | ASC_REQ_STREAM, SECURITY_NATIVE_DREP, isFirstCall ? tempCtxHandle.get() : nullptr, &outDesc, &retFlags, nullptr);
		isFirstCall = false;
		...
		{
			if (auto result = co_await ne::io::SendAwaitable{ stream.transport.Handle(), _engine }; result.IsError())
			{
			...
			const int sent = ::send(stream.transport.Handle(), static_cast<const char*>(outBuffers[0].pvBuffer), static_cast<int>(outBuffers[0].cbBuffer), 0);
			functionTable->FreeContextBuffer(outBuffers[0].pvBuffer);
			if (sent < 0)
				co_return ne::Result < TlsStream, ne::OsError > ::Error(ne::OsError{ ne::LastOsError() }.Context("[TlsStream/Accept/Send]"));
		}
...
} ne::Task<ne::Result<void, ne::OsError>> TlsStream::Handshake()
{
	...
	while (true)
	{
		...
		if (outBuffers[0].pvBuffer && outBuffers[0].cbBuffer > 0)
		{
			if (auto result = co_await ne::io::SendAwaitable{ transport.Handle(), transport.Engine() }; result.IsError())
			{
				functionTable->FreeContextBuffer(outBuffers[0].pvBuffer);
				co_return ne::Result<void, ne::OsError>::Error(std::move(result.Error()));
			}

			const int bytes = ::send(transport.Handle(), static_cast<const char*>(outBuffers[0].pvBuffer), static_cast<int>(outBuffers[0].cbBuffer), 0);
			functionTable->FreeContextBuffer(outBuffers[0].pvBuffer);
			if (bytes < 0)
				co_return ne::Result<void, ne::OsError>::Error(ne::OsError{ ne::LastOsError() }.Context("[TlsStream/Handshake/Send]"));
		}
		...
		if (ss == SEC_I_CONTINUE_NEEDED || ss == SEC_E_INCOMPLETE_MESSAGE)
		{
			...
			if (auto result = co_await ne::io::ReceiveAwaitable{ transport.Handle(), transport.Engine() }; result.IsError()) co_return ne::Result<void, ne::OsError>::Error(std::move(result.Error()));

			const int bytes = ::recv(transport.Handle(), reinterpret_cast<char*>(span.data() + dataInBuffer), static_cast<int>(span.size() - dataInBuffer), 0);
			if (bytes <= 0)
				co_return ne::Result<void, ne::OsError>::Error(ne::OsError{ ne::LastOsError() }.Context("[TlsStream/Handshake/Recv]"));
		...
		co_return ne::Result<void, ne::OsError>::Error(SchannelError(ss, "[TlsStream/Handshake]"));
	}
...
} ne::Task<ne::Result<std::size_t, ne::OsError>> TlsStream::Send(ne::io::BufferView _data)
{
	...
	if (ss != SEC_E_OK)
		co_return ne::Result<std::size_t, ne::OsError>::Error(SchannelError(ss, "[TlsStream/Send/QueryAttr]"));
	...
	while (totalSent < dataSpan.size())
	{
		...
		if (ss != SEC_E_OK)
			co_return ne::Result<std::size_t, ne::OsError>::Error(SchannelError(ss, "[TlsStream/Send/Encrypt]"));

		const ULONG encodeSize = buffers[0].cbBuffer + buffers[1].cbBuffer + buffers[2].cbBuffer;

		if (auto result = co_await ne::io::SendAwaitable{ transport.Handle(), transport.Engine() }; result.IsError()) co_return ne::Result<std::size_t, ne::OsError>::Error(std::move(result.Error()));

		const int bytes = ::send(transport.Handle(), reinterpret_cast<const char*>(encodeBuffer.data()), static_cast<int>(encodeSize), 0);
		if (bytes < 0)
			co_return ne::Result<std::size_t, ne::OsError>::Error(ne::OsError{ ne::LastOsError() }.Context("[TlsStream/Send]"));
...
} ne::Task<ne::Result<std::size_t, ne::OsError>> TlsStream::Receive(ne::io::BufferView _data)
{
	...
	while (true)
	{
		if (dataInBuffer == 0)
		{
			if (auto result = co_await ne::io::ReceiveAwaitable{ transport.Handle(), transport.Engine() }; result.IsError()) co_return ne::Result<std::size_t, ne::OsError>::Error(std::move(result.Error()));

			const int bytes = ::recv(transport.Handle(), reinterpret_cast<char*>(span.data()), static_cast<int>(span.size()), 0);
		...
		if (ss == SEC_I_CONTEXT_EXPIRED) co_return ne::Result<std::size_t, ne::OsError>::Ok(0);
		...
		if (ss == SEC_E_INCOMPLETE_MESSAGE)
		{
			...
			const int bytes = ::recv(transport.Handle(), reinterpret_cast<char*>(span.data() + dataInBuffer), static_cast<int>(span.size() - dataInBuffer), 0);
			if (bytes <= 0)
				co_return ne::Result<std::size_t, ne::OsError>::Error(ne::OsError{ ne::LastOsError() }.Context("[TlsStream/Receive/Recv]"));

			dataInBuffer += static_cast<std::size_t>(bytes);
			continue;
		}

		co_return ne::Result<std::size_t, ne::OsError>::Error(SchannelError(ss, "[TlsStream/Receive/Decrypt]"));
	}
...
static ne::OsError SslError(string_view_t _ctx)
{
	...
	err.Context(_ctx);
	return err;
} TlsStream::TlsStream(PlainStream&& _transport, void* _ctx, void* _ssl) noexcept
	: transport(std::move(_transport))
	, ctx(_ctx)
	, ssl(_ssl) {} TlsStream::TlsStream(TlsStream&& _other) noexcept
	: transport(std::move(_other.transport))
	, sniHost(std::move(_other.sniHost))
	, ctx(std::exchange(_other.ctx, nullptr))
	, ssl(std::exchange(_other.ssl, nullptr)) {} TlsStream& TlsStream::operator=(TlsStream&& _other) noexcept
{
	if (this != &_other)
	{
		if (ssl)
...
} ne::Task<ne::Result<TlsStream, ne::OsError>> TlsStream::Connect(Socket&& _socket, ne::io::IIoEngine& _engine, string_view_t _host, const TlsConfig& _config, ne::memory::IAllocator* _allocator)
{
	...
	if (transportRes.IsError())
	{
		SSL_free(tempSsl);
		SSL_CTX_free(sslCtx);
		co_return ne::Result < TlsStream, ne::OsError > ::Error(std::move(transportRes.Error()).Context("[TlsStream/Connect]"));
	}
	PlainStream transport = std::move(transportRes.Value());

	SSL_set_fd(tempSsl, static_cast<int>(transport.Handle()));
	if (!_host.empty()) SSL_set_tlsext_host_name(tempSsl, _host.data());

	TlsStream stream(std::move(transport), sslCtx, tempSsl);
	stream.sniHost = string_t(_host);

	auto result = co_await stream.Handshake();
	if (result.IsError()) co_return ne::Result < TlsStream, ne::OsError > ::Error(std::move(result.Error()));

	co_return ne::Result < TlsStream, ne::OsError > ::Ok(std::move(stream));
} ne::Task<ne::Result<TlsStream, ne::OsError>> TlsStream::Accept(Socket&& _socket, ne::io::IIoEngine& _engine, const TlsConfig& _config, ne::memory::IAllocator* _allocator)
{
	SSL_CTX* sslCtx = SSL_CTX_new(TLS_server_method());
	if (!sslCtx) co_return ne::Result < TlsStream, ne::OsError > ::Error(SslError("[TlsStream/Accept]"));
	...
	if (transportRes.IsError())
	{
		SSL_free(tempSsl);
		SSL_CTX_free(sslCtx);
		co_return ne::Result < TlsStream, ne::OsError > ::Error(std::move(transportRes.Error()).Context("[TlsStream/Accept]"));
	}
	PlainStream transport = std::move(transportRes.Value());
	...
	while (true)
	{
		...
		if (sslError == SSL_ERROR_WANT_READ) { if (auto result = co_await ne::io::ReceiveAwaitable{ stream.transport.Handle(), _engine }; result.IsError()) co_return ne::Result < TlsStream, ne::OsError > ::Error(std::move(result.Error())); }
		else if (sslError == SSL_ERROR_WANT_WRITE) { if (auto result = co_await ne::io::SendAwaitable{ stream.transport.Handle(), _engine }; result.IsError()) co_return ne::Result < TlsStream, ne::OsError > ::Error(std::move(result.Error())); }
		else { co_return ne::Result < TlsStream, ne::OsError > ::Error(SslError("[TlsStream/Accept/Handshake]")); }
	}
...
} ne::Task<ne::Result<void, ne::OsError>> TlsStream::Handshake()
{
	...
	while (true)
	{
		...
		if (sslError == SSL_ERROR_WANT_READ) { if (auto result = co_await ne::io::ReceiveAwaitable{ transport.Handle(), transport.Engine() }; result.IsError()) co_return ne::Result<void, ne::OsError>::Error(std::move(result.Error())); }
		else if (sslError == SSL_ERROR_WANT_WRITE) { if (auto result = co_await ne::io::SendAwaitable{ transport.Handle(), transport.Engine() }; result.IsError()) co_return ne::Result<void, ne::OsError>::Error(std::move(result.Error())); }
		else { co_return ne::Result<void, ne::OsError>::Error(SslError("[TlsStream/Handshake]")); }
	}
...
} ne::Task<ne::Result<std::size_t, ne::OsError>> TlsStream::Send(ne::io::BufferView _data)
{
	...
	while (sent < dataSpan.size())
	{
		...
		if (sslError == SSL_ERROR_WANT_WRITE) { if (auto result = co_await ne::io::SendAwaitable{ transport.Handle(), transport.Engine() }; result.IsError()) co_return ne::Result<std::size_t, ne::OsError>::Error(std::move(result.Error())); }
		else if (sslError == SSL_ERROR_WANT_READ) { if (auto result = co_await ne::io::ReceiveAwaitable{ transport.Handle(), transport.Engine() }; result.IsError()) co_return ne::Result<std::size_t, ne::OsError>::Error(std::move(result.Error())); }
...
} ne::Task<ne::Result<std::size_t, ne::OsError>> TlsStream::Receive(ne::io::BufferView _data)
{
	...
	while (true)
	{
		...
		const int sslError = SSL_get_error(nativeSsl, bytes);
		if (sslError == SSL_ERROR_WANT_READ) { if (auto result = co_await ne::io::ReceiveAwaitable{ transport.Handle(), transport.Engine() }; result.IsError()) co_return ne::Result<std::size_t, ne::OsError>::Error(std::move(result.Error())); }
		else if (sslError == SSL_ERROR_WANT_WRITE) { if (auto result = co_await ne::io::SendAwaitable{ transport.Handle(), transport.Engine() }; result.IsError()) co_return ne::Result<std::size_t, ne::OsError>::Error(std::move(result.Error())); }
...
TlsStream::TlsStream(PlainStream&& _transport, void*, void*) noexcept
	: transport(std::move(_transport)) {}
...
ne::Task<ne::Result<TlsStream, ne::OsError>> TlsStream::Connect(Socket&&, ne::io::IIoEngine&, string_view_t, const TlsConfig&, ne::memory::IAllocator*) { co_return ne::Result < TlsStream, ne::OsError > ::Error(NoTls("[TlsStream/Connect]")); }

ne::Task<ne::Result<TlsStream, ne::OsError>> TlsStream::Accept(Socket&&, ne::io::IIoEngine&, const TlsConfig&, ne::memory::IAllocator*) { co_return ne::Result < TlsStream, ne::OsError > ::Error(NoTls("[TlsStream/Accept]")); }



ne::Task<ne::Result<void, ne::OsError>> TlsStream::Handshake() { co_return ne::Result<void, ne::OsError>::Error(NoTls("[TlsStream/Handshake]")); }

ne::Task<ne::Result<std::size_t, ne::OsError>> TlsStream::Send(ne::io::BufferView) { co_return ne::Result<std::size_t, ne::OsError>::Error(NoTls("[TlsStream/Send]")); }

ne::Task<ne::Result<std::size_t, ne::OsError>> TlsStream::Receive(ne::io::BufferView) { co_return ne::Result<std::size_t, ne::OsError>::Error(NoTls("[TlsStream/Receive]")); }

ne::Task<ne::Result<std::size_t, ne::OsError>> TlsStream::Sendv(const ne::io::BufferChain&) { co_return ne::Result<std::size_t, ne::OsError>::Error(NoTls("[TlsStream/Sendv]")); }

ne::Task<ne::Result<void, ne::OsError>> TlsStream::Shutdown() { co_return ne::Result<void, ne::OsError>::Ok(); }

ne::Result<void, ne::OsError> TlsStream::Close() { return ne::Result<void, ne::OsError>::Ok(); }
