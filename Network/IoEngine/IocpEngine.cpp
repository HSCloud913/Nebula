...
BEGIN_NS(ne::network)
	...
	ne::Result<void, ne::OsError> IocpEngine::RunOnce(int_t _timeoutMs)
	{
		...
		const DWORD timeout = (_timeoutMs < 0)
								? INFINITE
								: static_cast<DWORD>(_timeoutMs);
		...
		if (overlapped == nullptr)
		{
			...
			if (!ok && ::GetLastError() == WAIT_TIMEOUT) return ne::Result<void, ne::OsError>::Ok();
		...
		{
			// 소켓 에러 or 연결 종료
			const auto lastOsError = LastOsError();
