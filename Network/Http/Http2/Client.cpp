...
BEGIN_NS(ne::network::http_2)
	...
	ne::Task<ne::Result<Http2Client, ne::HttpError>>
	Http2Client::Connect(std::unique_ptr<ne::network::IStream> _stream)
	{
		Http2Client client(std::move(_stream));

		auto pr = co_await client.SendPreface();
		if (pr.IsError())
			co_return ne::Result<Http2Client, ne::HttpError>::Error(std::move(pr.Error()));

		auto sr = co_await client.SendSettings();
		if (sr.IsError())
			co_return ne::Result<Http2Client, ne::HttpError>::Error(std::move(sr.Error()));

		co_return ne::Result<Http2Client, ne::HttpError>::Ok(std::move(client));
	}

	ne::Task<ne::Result<ne::network::HttpResponse, ne::HttpError>>
	Http2Client::Request(const ne::network::HttpRequest& _req)
	{
		...
		if (hr.IsError())
			co_return ne::Result<ne::network::HttpResponse, ne::HttpError>::Error(std::move(hr.Error()));
		...
		if (hasBody)
		{
			...
			if (dr.IsError())
				co_return ne::Result<ne::network::HttpResponse, ne::HttpError>::Error(std::move(dr.Error()));
		}

		// SETTINGS ACK 와 HEADERS 프레임 소비
		auto headersResult = co_await ReadHeaders();
		if (headersResult.IsError())
			co_return ne::Result<ne::network::HttpResponse, ne::HttpError>::Error(std::move(headersResult.Error()));
	...
	ne::Task<ne::Result<void, ne::HttpError>> Http2Client::SendPreface()
	{
		...
		auto r = co_await stream->Send(std::span<const ne::byte_t>(pref, kClientPreface.size()));
		if (r.IsError())
			co_return ne::Result<void, ne::HttpError>::Error(
				ne::HttpError{ r.Error().What() }.Context("[Http2Client/SendPreface]"));
	...
	ne::Task<ne::Result<void, ne::HttpError>> Http2Client::SendSettings()
	{
		...
		auto r = co_await stream->Send(std::span<const ne::byte_t>(frame));
		if (r.IsError())
			co_return ne::Result<void, ne::HttpError>::Error(
				ne::HttpError{ r.Error().What() }.Context("[Http2Client/SendSettings]"));
		co_return ne::Result<void, ne::HttpError>::Ok();
	}

	ne::Task<ne::Result<void, ne::HttpError>>
	Http2Client::SendWindowUpdate(uint32_t _streamId, int32_t _increment)
	{
		...
		if (r.IsError())
			co_return ne::Result<void, ne::HttpError>::Error(
				ne::HttpError{ r.Error().What() }.Context("[Http2Client/SendWindowUpdate]"));
		co_return ne::Result<void, ne::HttpError>::Ok();
	}
		...
		while (true)
		{
			...
			while (read < kFrameHeaderSize)
			{
				auto r = co_await stream->Receive(
					std::span<ne::byte_t>(hbuf.data() + read, kFrameHeaderSize - read));
				if (r.IsError())
					co_return ne::Result<ne::network::HttpHeaders, ne::HttpError>::Error(
						ne::HttpError{ r.Error().What() }.Context("[Http2Client/ReadHeaders]"));
				if (r.Value() == 0)
					co_return ne::Result<ne::network::HttpHeaders, ne::HttpError>::Error(
						ne::HttpError{ "connection closed" });
				read += r.Value();
			}
			...
			if (fh.type == FrameType::Headers)
			{
				...
				{
					auto r = co_await stream->Receive(
						std::span<ne::byte_t>(block.data() + blockRead, fh.length - blockRead));
					if (r.IsError())
						co_return ne::Result<ne::network::HttpHeaders, ne::HttpError>::Error(
							ne::HttpError{ r.Error().What() }.Context("[Http2Client/ReadHeaders/Block]"));
					blockRead += r.Value();
				}
				co_return ne::Result<ne::network::HttpHeaders, ne::HttpError>::Ok(
					Hpack::Decode(block.data(), block.size()));
			}
			...
			{
				auto r = co_await stream->Receive(
					std::span<ne::byte_t>(discard.data() + discarded, fh.length - discarded));
				if (r.IsError()) break;
				discarded += r.Value();
			}
			...
			if (fh.type == FrameType::Settings && !(fh.flags & Flag::Ack))
			{
				...
				(void)co_await stream->Send(std::span<const ne::byte_t>(ack));
			}
		}
	}
END_NS
