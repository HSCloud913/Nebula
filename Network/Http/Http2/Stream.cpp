...
BEGIN_NS(ne::network::http_2)
	                         ...
	                         const std::vector<ne::byte_t>& _hpackBlock)
	{
		...
		auto r = co_await transport->Send(std::span<const ne::byte_t>(header));
		if (r.IsError())
		...
		{
			auto r2 = co_await transport->Send(std::span<const ne::byte_t>(_hpackBlock));
			if (r2.IsError())
				co_return ne::Result<void, ne::HttpError>::Error(
	...
	Http2Stream::SendData(std::span<const ne::byte_t> _data, bool_t _endStream)
	{
		...
		if (!_data.empty())
		{
			auto r2 = co_await transport->Send(_data);
			if (r2.IsError())
				co_return ne::Result<void, ne::HttpError>::Error(
		...
		while (state != StreamState::HalfClosedRemote && state != StreamState::Closed)
		{
			...
			while (total < kFrameHeaderSize)
			{
				auto r = co_await transport->Receive(
					std::span<ne::byte_t>(headerBuf.data() + total, kFrameHeaderSize - total));
				if (r.IsError())
			...
			if (fh.type == FrameType::Data)
			{
				...
				{
					auto r = co_await transport->Receive(
						std::span<ne::byte_t>(payload.data() + read, fh.length - read));
					if (r.IsError())
						co_return ne::Result<ne::network::HttpResponse, ne::HttpError>::Error(
			...
			else
			{
				...
				{
					auto r = co_await transport->Receive(
						std::span<ne::byte_t>(discard.data() + dread, fh.length - dread));
					if (r.IsError()) break;
					dread += r.Value();
				}
				if (fh.flags & Flag::EndStream)
