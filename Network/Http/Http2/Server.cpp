...
BEGIN_NS(ne::network::http_2)
	...
	ne::Task<ne::Result<void, ne::HttpError>> Http2Server::Run()
	{
		...
		while (read < kClientPreface.size())
		{
			auto r = co_await stream->Receive(
				std::span<ne::byte_t>(prefaceBuf.data() + read, kClientPreface.size() - read));
			if (r.IsError())
		...
		while (true)
		{
			...
			{
				auto r = co_await stream->Receive(
					std::span<ne::byte_t>(hbuf.data() + hr, kFrameHeaderSize - hr));
				if (r.IsError())
					co_return ne::Result<void, ne::HttpError>::Error(
			...
			{
				auto r = co_await stream->Receive(
					std::span<ne::byte_t>(payload.data() + pr, fh.length - pr));
				if (r.IsError())
					co_return ne::Result<void, ne::HttpError>::Error(
	...
	Http2Server::SendHeaders(uint32_t _streamId, const ne::network::HttpResponse& _resp)
	{
		...
		auto r = co_await stream->Send(std::span<const ne::byte_t>(hdr));
		if (r.IsError())
			co_return ne::Result<void, ne::HttpError>::Error(
		...
		if (!block.empty())
		{
			auto r2 = co_await stream->Send(std::span<const ne::byte_t>(block));
			if (r2.IsError())
				co_return ne::Result<void, ne::HttpError>::Error(
	...
	Http2Server::SendData(uint32_t _streamId, const ne::string_t& _body)
	{
		...
		auto r2 = co_await stream->Send(std::span<const ne::byte_t>(body, _body.size()));
		if (r2.IsError())
			co_return ne::Result<void, ne::HttpError>::Error(
