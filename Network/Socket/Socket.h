...
[[nodiscard]] constexpr bool_t HasFlag(const SocketCreateFlags _set, const SocketCreateFlags _bit) noexcept { return (static_cast<uint32_t>(_set) & static_cast<uint32_t>(_bit)) == static_cast<uint32_t>(_bit); }
...
private:
	#if defined(_WIN32)
	using SocketHandle = ne::Handle<socket_t, decltype([](const socket_t _socket) { ::closesocket(_socket); }), INVALID_SOCKET>;
	#elif defined(IS_POSIX)
	using SocketHandle = ne::Handle<socket_t, decltype([](const socket_t _socket) { ::close(_socket); }), static_cast<socket_t>(-1)>;
	#endif
