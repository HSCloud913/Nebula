...
Result<Socket, OsError> Socket::Create(const AddressFamily _family, const int_t _type, const int_t _protocol, const SocketCreateFlags _flags)
{
	...
	if (fd == InvalidSocket)
		return ne::Result < Socket, ne::OsError > ::Error(ne::OsError{ LastOsError() }.Context("[Socket/Create]"));
...
ne::Result<void, ne::OsError> Socket::SetReuseAddress(const bool_t _enable)
{
	...
	if (::setsockopt(handle.Get(), SOL_SOCKET, SO_REUSEADDR, reinterpret_cast<const char_t*>(&value), sizeof(value)) != 0) { return ne::Result<void, ne::OsError>::Error(ne::OsError{ LastOsError() }.Context("[Socket/SetReuseAddress]")); }
...
ne::Result<void, ne::OsError> Socket::SetNoDelay(const bool_t _enable)
{
	...
	if (::setsockopt(handle.Get(), IPPROTO_TCP, TCP_NODELAY, reinterpret_cast<const char_t*>(&value), sizeof(value)) != 0)
		return ne::Result<void, ne::OsError>::Error(ne::OsError{ LastOsError() }.Context("[Socket/SetNoDelay]"));
...
{
	#if defined(_WIN32)
	ulong_t mode = _enable ? 1u : 0u; if (::ioctlsocket(handle.Get(), FIONBIO, &mode) != 0)
		return ne::Result<void, ne::OsError>::Error(ne::OsError{ LastOsError() }.Context("[Socket/SetNonBlocking]"));
	#elif defined(IS_POSIX)
	int_t flags = ::fcntl(handle.Get(), F_GETFL, 0); if (flags == -1)
		return ne::Result<void, ne::OsError>::Error(ne::OsError{ errno }.Context("[Socket/SetNonBlocking]")); flags = _enable ? (flags | O_NONBLOCK) : (flags & ~O_NONBLOCK); if (::fcntl(handle.Get(), F_SETFL, flags) == -1)
		return ne::Result<void, ne::OsError>::Error(ne::OsError{ errno }.Context("[Socket/SetNonBlocking]"));
	#endif
...
{
	#if defined(_WIN32)
	const ulong_t ms = _timeout.count(); if (::setsockopt(handle.Get(), SOL_SOCKET, SO_SNDTIMEO, reinterpret_cast<const char_t*>(&ms), sizeof(ms)) != 0)
		return ne::Result<void, ne::OsError>::Error(ne::OsError{ LastOsError() }.Context("[Socket/SetSendTimeout]"));

	#elif defined(IS_POSIX)
	const timeval tv{ .tv_sec = static_cast<time_t>(_timeout.count() / 1000), .tv_usec = static_cast<suseconds_t>((_timeout.count() % 1000) * 1000) }; if (::setsockopt(handle.Get(), SOL_SOCKET, SO_SNDTIMEO, reinterpret_cast<const char_t*>(&tv), sizeof(tv)) != 0)
		return ne::Result<void, ne::OsError>::Error(ne::OsError{ errno }.Context("[Socket/SetSendTimeout]"));
...
{
	#if defined(_WIN32)
	const ulong_t ms = _timeout.count(); if (::setsockopt(handle.Get(), SOL_SOCKET, SO_RCVTIMEO, reinterpret_cast<const char_t*>(&ms), sizeof(ms)) != 0)
		return ne::Result<void, ne::OsError>::Error(ne::OsError{ LastOsError() }.Context("[Socket/SetReceiveTimeout]"));

	#elif defined(IS_POSIX)
	const timeval tv{ .tv_sec = static_cast<time_t>(_timeout.count() / 1000), .tv_usec = static_cast<suseconds_t>((_timeout.count() % 1000) * 1000) }; if (::setsockopt(handle.Get(), SOL_SOCKET, SO_RCVTIMEO, reinterpret_cast<const char_t*>(&tv), sizeof(tv)) != 0)
		return ne::Result<void, ne::OsError>::Error(ne::OsError{ errno }.Context("[Socket/SetReceiveTimeout]"));
...
ne::Task<ne::Result<void, ne::OsError>> Socket::Bind(const string_view_t _address, const uint16_t _port)
{
	...
	if (::bind(handle.Get(), reinterpret_cast<const sockaddr*>(&resolved.Value()), resolved.Value().ss_family == AF_INET6 ? sizeof(sockaddr_in6) : sizeof(sockaddr_in)) != 0)
		co_return ne::Result<void, ne::OsError>::Error(ne::OsError{ LastOsError() }.Context("[Socket/Bind]"));
...
ne::Result<void, ne::OsError> Socket::Listen(const int_t _backlog)
{
	if (::listen(handle.Get(), _backlog) != 0)
		return ne::Result<void, ne::OsError>::Error(ne::OsError{ LastOsError() }.Context("[Socket/Listen]"));
...
ne::Result<Socket, ne::OsError> Socket::Accept()
{
	...
	if (clientFd == InvalidSocket)
		return ne::Result < Socket, ne::OsError > ::Error(ne::OsError{ LastOsError() }.Context("[Socket/Accept]"));
...
ne::Result<sockaddr_storage, ne::OsError> Socket::ResolveAddress(const string_view_t _address, const uint16_t _port) const
{
	...
	if (!candidates) return ne::Result<sockaddr_storage, ne::OsError>::Error(std::move(candidates.Error()));
...
ne::Result<std::vector<sockaddr_storage>, ne::OsError> Socket::ResolveCandidates(const string_view_t _address, const uint16_t _port) const
{
	...
	if (::getaddrinfo(addressString.c_str(), nullptr, &hints, &result) != 0)
		return ne::Result<std::vector<sockaddr_storage>, ne::OsError>::Error(ne::OsError{ LastOsError() }.Context("[Socket/ResolveCandidates]"));
...
ne::Result<void, ne::OsError> Socket::ConnectResolved(const std::vector<sockaddr_storage>& _candidates)
{
	...
	for (std::size_t i = 0; i < _candidates.size(); ++i)
	{
		...
		if (::connect(handle.Get(), reinterpret_cast<const sockaddr*>(&candidate), candidate.ss_family == AF_INET6 ? sizeof(sockaddr_in6) : sizeof(sockaddr_in)) == 0) return ne::Result<void, ne::OsError>::Ok();

		const ulong_t lastOsError = LastOsError();
		#if defined(_WIN32)
		if (lastOsError == WSAEINPROGRESS || lastOsError == WSAEWOULDBLOCK) return ne::Result<void, ne::OsError>::Ok();
		#elif defined(IS_POSIX)
		if (errno == EINPROGRESS) return ne::Result<void, ne::OsError>::Ok();
		#endif
...
ne::Task<ne::Result<void, ne::OsError>> Socket::ConnectResolvedAsync(const std::vector<sockaddr_storage>& _candidates, ne::io::IIoEngine& _engine)
{
	...
	for (std::size_t i = 0; i < _candidates.size(); ++i)
	{
		...
		const auto& candidate = _candidates[i];
		if (::connect(handle.Get(), reinterpret_cast<const sockaddr*>(&candidate), candidate.ss_family == AF_INET6 ? sizeof(sockaddr_in6) : sizeof(sockaddr_in)) == 0) co_return ne::Result<void, ne::OsError>::Ok(); // 즉시 연결(로컬 루프백 등 드문 케이스)
		...
		if (soError == 0) co_return ne::Result<void, ne::OsError>::Ok();
