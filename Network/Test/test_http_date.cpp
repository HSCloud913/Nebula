...
{
	EXPECT_EQ(FormatDate(Utc(1994, 11, 6, 8, 49, 37)), "Sun, 06 Nov 1994 08:49:37 GMT");
}

TEST(HttpDateTest, FormatsEpoch)
{
	EXPECT_EQ(FormatDate(Utc(1970, 1, 1, 0, 0, 0)), "Thu, 01 Jan 1970 00:00:00 GMT");
}
...
TEST(HttpDateTest, RoundTripsThroughParse)
{
	...
	{
		const string_t text = FormatDate(value);
		const auto parsed = ParseDate(text);
...
{
	const auto parsed = ParseDate("Sun, 06 Nov 1994 08:49:37 GMT");
...
{
	const auto parsed = ParseDate("Mon, 06 Nov 1994 08:49:37 GMT"); // 실제로는 Sunday
	ASSERT_TRUE(parsed.has_value());
	EXPECT_EQ(*parsed, Utc(1994, 11, 6, 8, 49, 37));
}
...
TEST(HttpDateTest, RejectsMalformedInput)
{
	...
	EXPECT_FALSE(ParseDate("Sun, 06 Nov 1994 08:49:37").has_value());        // GMT 누락
	EXPECT_FALSE(ParseDate("Sun, 06 Nov 1994 08:49:37 UTC").has_value());    // GMT 가 아님
	EXPECT_FALSE(ParseDate("Sun  06 Nov 1994 08:49:37 GMT").has_value());    // 쉼표 누락
	EXPECT_FALSE(ParseDate("Xxx, 06 Nov 1994 08:49:37 GMT").has_value());    // 요일 이름 오류
	EXPECT_FALSE(ParseDate("Sun, 06 Xxx 1994 08:49:37 GMT").has_value());    // 월 이름 오류
	EXPECT_FALSE(ParseDate("Sun, 6 Nov 1994 08:49:37 GMT").has_value());     // 일이 2자리가 아님
	EXPECT_FALSE(ParseDate("Sun, 06 Nov 1994 24:49:37 GMT").has_value());    // 시 범위 초과
	EXPECT_FALSE(ParseDate("Sun, 06 Nov 1994 08:60:37 GMT").has_value());    // 분 범위 초과
	EXPECT_FALSE(ParseDate("Wed, 31 Feb 1994 08:49:37 GMT").has_value());    // 존재하지 않는 날짜
	EXPECT_FALSE(ParseDate("Sun, 29 Feb 1900 08:49:37 GMT").has_value());    // 1900 은 윤년이 아니다
	EXPECT_FALSE(ParseDate("Sun, 0a Nov 1994 08:49:37 GMT").has_value());    // 숫자가 아닌 문자
}

// ── 윤초 표기(초 60)는 59 로 클램프해 받아들인다(system_clock 에는 윤초가 없다) ──
TEST(HttpDateTest, ClampsLeapSecond)
{
	const auto parsed = ParseDate("Sun, 31 Dec 1995 23:59:60 GMT");
	ASSERT_TRUE(parsed.has_value());
...
{
	EXPECT_TRUE(ParseDate("Tue, 29 Feb 2000 12:00:00 GMT").has_value()); // 400 배수 → 윤년
	EXPECT_TRUE(ParseDate("Mon, 29 Feb 2016 12:00:00 GMT").has_value());
	EXPECT_FALSE(ParseDate("Sun, 29 Feb 2100 12:00:00 GMT").has_value()); // 100 배수, 400 배수 아님
}
...
TEST(HttpDateTest, RejectsYearsOutsideFourDigits)
{
	...
	EXPECT_TRUE(FormatDate(system_clock::time_point::max()).empty());

	// 표현 범위 안의 정상 시각은 항상 고정 폭 29자다.
	EXPECT_EQ(FormatDate(Utc(2026, 8, 11, 12, 0, 0)).size(), 29u);
}
...
{
	EXPECT_FALSE(ParseDate("Sun, 06 Nov 1994 -8:49:37 GMT").has_value());
	EXPECT_FALSE(ParseDate("Sun, 06 Nov 1994 08:-9:37 GMT").has_value());
	EXPECT_FALSE(ParseDate("Sun, 06 Nov 1994 08:49:-7 GMT").has_value());
	EXPECT_FALSE(ParseDate("Sun, -6 Nov 1994 08:49:37 GMT").has_value());
	EXPECT_FALSE(ParseDate("Sun, 06 Nov -123 08:49:37 GMT").has_value());
}
