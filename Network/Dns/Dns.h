...
BEGIN_NS(ne::network::dns)
	// getaddrinfo 결과 하나. family 는 AF_INET/AF_INET6(플랫폼 소켓 API 값 그대로) — io::Socket::Create
	// 에 바로 넘길 수 있다.
	struct Candidate
	{
		int_t family{};
	...
	// _host 가 IP 리터럴(v4/v6)이면 파싱만으로, 호스트명이면 DNS 조회로 후보 전부(A/AAAA)를
	// 돌려준다. 블로킹 조회(getaddrinfo)는 전용 워커 스레드로 오프로드해 호출 스레드(이벤트 루프)
	// 를 막지 않는다.
