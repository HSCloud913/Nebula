...
BEGIN_NS(ne::network)
	...
	{
	private:
		BufferBlock(ne::byte_t* _data, std::size_t _capacity, ne::memory::IAllocator* _pool) noexcept;
	...
	public:
		[[nodiscard]] static ne::Result<BufferBlock*, ne::OsError> Acquire(ne::memory::IAllocator& _pool, std::size_t _size, std::size_t _align = alignof(std::max_align_t)) noexcept;
