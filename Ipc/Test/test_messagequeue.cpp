...
TEST(MessageQueueTest, ReceiveAsyncBasic)
{
	...
	TestIoEngine engine; auto server = MessageQueue("nebula-mq-async-basic"); auto serverListenThread = std::thread([&server] { server.Listen(); }); auto client = MessageQueue("nebula-mq-async-basic"); client.Connect(); serverListenThread.join(); const auto message = std::string("nebula-async-receive"); std::atomic<bool> done{ false }; ne::Result<std::vector<std::byte>, ne::OsError> received = ne::Result<std::vector<std::byte>, ne::OsError>::Ok({}); auto task = [&]() -> ne::Task<void>
	{
	...
	std::thread([&client, &message]
	{
		...
		client.Send(AsBytes(message));
	}).detach(); DriveEngine(engine, done); ASSERT_TRUE(done.load()) << "ReceiveAsync timed out"; ASSERT_TRUE(received.IsOk()) << received.Error().What(); EXPECT_EQ(AsString(received.Value()), message);
	#else
...
{
	#if defined(IS_POSIX) || defined(_WIN32)
	TestIoEngine engine; auto server = MessageQueue("nebula-mq-async-nonblock"); auto serverListenThread = std::thread([&server] { server.Listen(); }); auto client = MessageQueue("nebula-mq-async-nonblock"); client.Connect(); serverListenThread.join(); std::atomic<bool> aDone{ false }; std::atomic<bool> bDone{ false }; ne::Result<std::vector<std::byte>, ne::OsError> aResult = ne::Result<std::vector<std::byte>, ne::OsError>::Ok({});
	...
	}).detach(); DriveEngine(engine, aDone); ASSERT_TRUE(aDone.load()) << "ReceiveAsync timed out"; ASSERT_TRUE(aResult.IsOk()) << aResult.Error().What(); EXPECT_EQ(AsString(aResult.Value()), "ping");
	#else
	GTEST_SKIP() << "Platform not supported";
	#endif
}
...
{
	#if defined(IS_POSIX) || defined(_WIN32)
	TestIoEngine engine; auto server = MessageQueue("nebula-mq-async-send"); auto serverListenThread = std::thread([&server] { server.Listen(); }); auto client = MessageQueue("nebula-mq-async-send"); client.Connect(); serverListenThread.join(); const auto message = std::string("nebula-async-send"); std::atomic<bool> done{ false }; ne::Result<void, ne::OsError> sendResult = ne::Result<void, ne::OsError>::Ok(); auto task = [&]() -> ne::Task<void>
	{
		sendResult = co_await client.SendAsync(AsBytes(message), engine);
		done.store(true, std::memory_order_release);
	}(); task.Resume(); DriveEngine(engine, done); ASSERT_TRUE(done.load()) << "SendAsync timed out"; ASSERT_TRUE(sendResult.IsOk()) << sendResult.Error().What(); const auto received = server.Receive(); EXPECT_EQ(AsString(received), message);
	#else
	GTEST_SKIP() << "Platform not supported";
	#endif
}
