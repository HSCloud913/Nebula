...
namespace
{
	...
	void DriveEngine(TestIoEngine& _engine, const std::atomic<bool>& _flag, const std::chrono::milliseconds _timeout = std::chrono::seconds(5))
	{
		const auto deadline = std::chrono::steady_clock::now() + _timeout;
		while (!_flag.load(std::memory_order_acquire) && std::chrono::steady_clock::now() < deadline) (void)_engine.RunOnce(10);
	}
}
...
// ReadAsync 가 IEngine 기반으로 정상 수신하는지 검증.
...
TEST(PipeTest, ReadAsyncBasic)
{
	...
	TestIoEngine engine; auto server = Pipe("nebula-pipe-async-basic"); auto serverListenThread = std::thread([&server] { server.Listen(); }); auto client = Pipe("nebula-pipe-async-basic"); client.Connect(); serverListenThread.join(); const auto message = std::string("nebula-async-receive"); std::atomic<bool> done{ false }; ne::Result<std::size_t, ne::OsError> received = ne::Result<std::size_t, ne::OsError>::Ok(0); std::vector<std::byte> buffer(64); auto task = [&]() -> ne::Task<void>
	{
	...
	}).detach(); DriveEngine(engine, done); ASSERT_TRUE(done.load()) << "ReadAsync timed out"; ASSERT_TRUE(received.IsOk()) << received.Error().What(); buffer.resize(received.Value()); EXPECT_EQ(AsString(buffer), message);
	#else
...
{
	#if defined(IS_POSIX) || defined(_WIN32)
	TestIoEngine engine; auto server = Pipe("nebula-pipe-async-nonblock"); auto serverListenThread = std::thread([&server] { server.Listen(); }); auto client = Pipe("nebula-pipe-async-nonblock"); client.Connect(); serverListenThread.join(); std::atomic<bool> aDone{ false }; std::atomic<bool> bDone{ false }; ne::Result<std::size_t, ne::OsError> aResult = ne::Result<std::size_t, ne::OsError>::Ok(0); std::vector<std::byte> buffer(64);
	...
	EXPECT_TRUE(bDone.load(std::memory_order_acquire)) << "B must run while A is suspended";
	...
	}).detach(); DriveEngine(engine, aDone); ASSERT_TRUE(aDone.load()) << "ReadAsync timed out"; ASSERT_TRUE(aResult.IsOk()) << aResult.Error().What(); buffer.resize(aResult.Value()); EXPECT_EQ(AsString(buffer), "ping");
	#else
	GTEST_SKIP() << "Platform not supported";
	#endif
}
...
{
	#if defined(IS_POSIX) || defined(_WIN32)
	TestIoEngine engine; auto server = Pipe("nebula-pipe-async-write"); auto serverListenThread = std::thread([&server] { server.Listen(); }); auto client = Pipe("nebula-pipe-async-write"); client.Connect(); serverListenThread.join(); const auto message = std::string("nebula-async-send"); std::atomic<bool> done{ false }; ne::Result<std::size_t, ne::OsError> writeResult = ne::Result<std::size_t, ne::OsError>::Ok(0); auto task = [&]() -> ne::Task<void>
	{
		writeResult = co_await client.WriteAsync(AsBytes(message), engine);
		done.store(true, std::memory_order_release);
	}(); task.Resume(); DriveEngine(engine, done); ASSERT_TRUE(done.load()) << "WriteAsync timed out"; ASSERT_TRUE(writeResult.IsOk()) << writeResult.Error().What(); EXPECT_EQ(writeResult.Value(), message.size()); auto buffer = std::vector<std::byte>(64); const auto received = server.Read(buffer); ASSERT_GE(received, 0); buffer.resize(received); EXPECT_EQ(AsString(buffer), message);
	#else
	GTEST_SKIP() << "Platform not supported";
	#endif
}
