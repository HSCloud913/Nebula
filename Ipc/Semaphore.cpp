...
public:
	void_t Acquire() const { if (::WaitForSingleObject(handle, INFINITE) != WAIT_OBJECT_0) { throw ne::Exception("[Semaphore/Acquire]", std::format("Failed to WaitForSingleObject function (error: {})", ::GetLastError())); } }

	[[nodiscard]] bool_t TryAcquire() const
	{
		if (const auto result = ::WaitForSingleObject(handle, 0); result == WAIT_OBJECT_0) { return true; }
	...
	void_t Release(const int_t _count) const { if (!::ReleaseSemaphore(handle, _count, nullptr)) { throw ne::Exception("[Semaphore/Release]", std::format("Failed to ReleaseSemaphore function (error: {})", ::GetLastError())); } }
};
...
public:
		...
		: name("/" + string_t(_name))
	{
		...
		if (handle == SEM_FAILED) { throw ne::Exception("[Semaphore/Impl]", std::format("Failed to sem_open function (error: {})", errno)); }
	}
	~Impl()
	{
...
public:
	void_t Acquire() const { while (::sem_wait(handle) == -1) { if (errno != EINTR) { throw ne::Exception("[Semaphore/Acquire]", std::format("Failed to sem_wait function (error: {})", errno)); } } }
	...
	void_t Release(const int_t _count) const { for (auto i = 0; i < _count; ++i) { if (::sem_post(handle) == -1) { throw ne::Exception("[Semaphore/Release]", std::format("Failed to sem_post function (error: {})", errno)); } } }
};
...
void_t Semaphore::Acquire() const { impl->Acquire(); }

bool_t Semaphore::TryAcquire() const { return impl->TryAcquire(); }
