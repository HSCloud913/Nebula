...
namespace
{
	// 완료를 기다리는 최소 awaitable — 프레임에 IoCompletionHandler 를 보관해 userData 로 넘긴다.
	...
	ne::Task<longlong_t> SubmitOp(IoContext& _context, IoRequest _request)
	{
	...
	ne::Task<int_t> PostRoundTrip(IoContext& _context)
	{
	...
	template <typename T>
	T DriveUntilReady(IoContext& _context, ne::Task<T>& _task)
	{
		_task.Resume();
...
TEST(IoContextTest, DispatchesCompletionToCoroutine)
{
	...
	IoRequest write{ .op = OpCode::Write, .handle = reinterpret_cast<ulonglong_t>(file),
	...
	char buffer[64]{};
	IoRequest read{ .op = OpCode::Read, .handle = reinterpret_cast<ulonglong_t>(file),
...
TEST(IoContextTest, TimerWheelIntegration)
{
	...
	(void)wheel.Schedule(std::chrono::milliseconds{ 50 }, [&] { fired.store(true, std::memory_order_release); });

	const auto start = std::chrono::steady_clock::now();
	while (!fired.load(std::memory_order_acquire) && std::chrono::steady_clock::now() - start < std::chrono::seconds(2))
		(void)context.RunOnce(std::chrono::milliseconds{ -1 }); // 타이머가 유효 타임아웃을 만든다
