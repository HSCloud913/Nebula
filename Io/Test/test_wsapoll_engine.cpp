...
TEST(WsaPollEngineTest, AcceptConnectRoundTrip)
{
	...
	int_t acceptTag = 0;
	Request accept{ .requestKind = RequestKind::ACCEPT, .userData = &acceptTag, .handle = static_cast<ulonglong_t>(listener) };
	engine.Submit(accept);

	int_t connectTag = 0;
	Request connect{ .requestKind = RequestKind::CONNECT, .userData = &connectTag, .handle = static_cast<ulonglong_t>(client), .address = &address, .addressLength = addressLength };
