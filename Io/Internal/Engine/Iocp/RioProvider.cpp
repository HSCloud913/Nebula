...
#include "Io/Internal/Engine/Iocp/RioProvider.h"
...
namespace ne::io
{
	IoResult<BufferHandle> RioProvider::RegisterBuffer(std::span<ne::byte_t> _region) noexcept
	{
		if (_region.empty()) return IoResult<BufferHandle>::Error(IoError{ IoErrorKind::INVALID_BUFFER, "empty region" });

		std::lock_guard lock(mutex);

		if (auto init = EnsureInitialized(); init.IsError()) return IoResult<BufferHandle>::Error(std::move(init.Error()));
		...
		if (id == RIO_INVALID_BUFFERID) return IoResult<BufferHandle>::Error(IoError{ ne::OsError{ ne::LastOsError() } }.Context("[RioProvider/RIORegisterBuffer]"));
		...
		return IoResult<BufferHandle>::Ok(handle);
	}
	...
	IoResult<void_t> RioProvider::EnsureInitialized() noexcept
	{
		if (isInitialized) return IoResult<void_t>::Ok();
		...
		if (socket == INVALID_SOCKET) return IoResult<void_t>::Error(IoError{ ne::OsError{ ne::LastOsError() } }.Context("[RioProvider/WSASocketW]"));
		...
		if (result == SOCKET_ERROR || !table.RIORegisterBuffer || !table.RIOCreateCompletionQueue) return IoResult<void_t>::Error(IoError{ ne::OsError{ ne::LastOsError() } }.Context("[RioProvider/WSAIoctl(RIO)]"));
		...
		if (cq == RIO_INVALID_CQ) return IoResult<void_t>::Error(IoError{ ne::OsError{ ne::LastOsError() } }.Context("[RioProvider/RIOCreateCompletionQueue]"));
	...
	IoResult<RIO_RQ> RioProvider::EnsureRequestQueueLocked(const socket_t _socket) noexcept
	{
		if (const auto requestQueue = requestQueues.find(_socket); requestQueue != requestQueues.end()) return IoResult<RIO_RQ>::Ok(requestQueue->second);
		...
		if (rq == RIO_INVALID_RQ) return IoResult<RIO_RQ>::Error(IoError{ ne::OsError{ ne::LastOsError() } }.Context("[RioProvider/RIOCreateRequestQueue]"));

		requestQueues[_socket] = rq;
		return IoResult<RIO_RQ>::Ok(rq);
	}
	...
	IoResult<void_t> RioProvider::Submit(const socket_t _socket, const BufferHandle _handle, const void_t* _buffer, const std::size_t _length, void_t* _userData, const bool_t _isSend) noexcept
	{
		std::lock_guard lock(mutex);

		const RIO_BUF buffer = MakeRioBufferLocked(_handle, _buffer, _length);
		if (!buffer.BufferId) return IoResult<void_t>::Error(IoError{ IoErrorKind::INVALID_BUFFER, "unregistered buffer handle" });

		if (auto init = EnsureInitialized(); init.IsError()) return IoResult<void_t>::Error(std::move(init.Error()));

		auto rq = EnsureRequestQueueLocked(_socket);
		if (rq.IsError()) return IoResult<void_t>::Error(std::move(rq.Error()));
		...
		if (!isOk) return IoResult<void_t>::Error(IoError{ ne::OsError{ ne::LastOsError() } }.Context(_isSend ? "[RioProvider/RIOSend]" : "[RioProvider/RIOReceive]"));
	...
	IoResult<void_t> RioProvider::ArmNotify() noexcept
	{
		if (!isInitialized || !table.RIONotify) return IoResult<void_t>::Error(IoError{ IoErrorKind::UNSUPPORTED, "RIO CQ not initialized" });

		if (const int result = table.RIONotify(cq); result != 0) return IoResult<void_t>::Error(IoError{ ne::OsError{ static_cast<ne::ulong_t>(result) } }.Context("[RioProvider/RIONotify]"));
