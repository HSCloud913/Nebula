...
namespace ne::io
{
	...
	bool_t IocpEngine::EnsureAssociated(const HANDLE _handle) noexcept
	{
		...
		// associated 셋으로 추적해 최초 1회만 시도한다. 닫힌 핸들 값의 재사용은 Deregister() 로
		// 셋에서 제거되어, 같은 값의 새 핸들이 다시 연관될 수 있다.
	...
	void_t IocpEngine::Dispatch(IocpOperation* _operation, const Request& _request, const HANDLE _handle) noexcept
	{
		...
		switch (_request.requestKind)
		{
			...
			case RequestKind::WAIT_READABLE:
			{
				...
				if (::WSARecv(reinterpret_cast<SOCKET>(_handle), &wsaBuffer, 1, &received, &flags, &_operation->overlapped, nullptr) == SOCKET_ERROR) if (const int_t error = ::WSAGetLastError(); error != WSA_IO_PENDING) syncError = error;
				break;
			}
			case RequestKind::WAIT_WRITABLE:
			{
				// IOCP 에는 "쓰기 준비" 통지가 없다. 예전에는 즉시 성공을 반환했지만, 그건 커널에 묻지 않고
				// 준비됐다고 거짓말하는 것이어서 호출자가 그 뒤 블로킹 send 를 하도록 유도했다(이벤트 루프
				// 정지). 프로액터에서는 준비 대기 대신 Send 를 바로 제출하면 되므로, 지원하지 않음을
				// 정직하게 알린다. WAIT_READABLE 은 0바이트 WSARecv 로 진짜 근사가 가능해 그대로 둔다.
