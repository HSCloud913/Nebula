...
namespace ne::io
{
	...
	public: /* IEngine */
		...
		/**
		 * @brief 닫히는 fd 의 readWaiter/writeWaiter/pending 상태를 정리하고, 진행 중이던 op 는 합성
		 * 완료(ECANCELED)로 회수한다. fd 값 재사용 시 stale waiter 엔트리가 새 소켓의 등록을 덮어쓰는
		 * 오염을 막고, abandon 된 채 완료가 오지 않아 누수되던 핸들러를 루프가 해제하게 한다.
		 */
