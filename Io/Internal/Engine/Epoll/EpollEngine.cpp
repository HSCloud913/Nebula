...
#include "Io/Internal/Engine/Iovec.h"
...
namespace ne::io
{
	...
	bool_t EpollEngine::Perform(const Request& _request, const bool_t _isRetry, longlong_t& _result) noexcept
	{
		...
		switch (_request.requestKind)
		{
			...
			case RequestKind::READ:
			{
				...
				{
					const auto iov = internal::ToIovec(*_request.chain);
			...
			case RequestKind::RECEIVE:
			{
				if (_request.chain != nullptr)
				{
					auto iov = internal::ToIovec(*_request.chain);
		...
		// EINTR 은 시그널(프로파일러/타이머 등)에 끊긴 것뿐이라 사용자에게 I/O 실패로 보이면 안 된다.
		// 준비 대기로 되돌리면 다음 통지에서 같은 op 를 재시도하게 된다.
	...
	void_t EpollEngine::Deregister(const ulonglong_t _handleKey) noexcept
	{
		...
		// 관심에서 명시적으로 제거한다(등록된 적 없으면 ENOENT 로 무해하게 실패).
		(void_t)::epoll_ctl(epollFd, EPOLL_CTL_DEL, fd, nullptr);
	}
