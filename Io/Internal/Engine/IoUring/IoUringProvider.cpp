...
namespace ne::io
{
	IoResult<BufferHandle> IoUringProvider::RegisterBuffer(const std::span<ne::byte_t> _region) noexcept
	{
		if (_region.empty()) return IoResult<BufferHandle>::Error(IoError{ IoErrorKind::INVALID_BUFFER, "empty region" });

		std::lock_guard lock(mutex);

		if (!EnsureSparseRegisteredLocked()) return IoResult<BufferHandle>::Error(IoError{ ne::OsError{ ne::LastOsError() } }.Context("[IoUringProvider/io_uring_register_buffers_sparse]"));
		...
		if (slot == MaxBuffers) return IoResult<BufferHandle>::Error(IoError{ IoErrorKind::REGISTRATION_LIMIT_EXCEEDED });

		// update_tag 는 sparse 로 예약해 둔 슬롯 하나만 골라 실제 iovec 을 채워 넣는 갱신 API 다.
		iovec iov{ _region.data(), _region.size() };
		__u64 tag = 0;
		if (::io_uring_register_buffers_update_tag(ring, slot, &iov, &tag, 1) != 0) return IoResult<BufferHandle>::Error(IoError{ ne::OsError{ ne::LastOsError() } }.Context("[IoUringProvider/io_uring_register_buffers_update_tag]"));

		usedSlots[slot] = true;
		// 0은 BufferHandle::IsValid() 기준 "무효" 이므로, 실제 슬롯 인덱스에 1을 더해 핸들 값으로 사용한다.
		return IoResult<BufferHandle>::Ok(BufferHandle{ static_cast<uint64_t>(slot) + 1 });
	}
