...
namespace ne::io
{
	...
	public: /* IRegisteredBufferProvider */
		 ...
		 */
		[[nodiscard]] virtual IoResult<BufferHandle> RegisterBuffer(std::span<ne::byte_t> _region) noexcept override;
	...
	public: /* IRegisteredBufferProvider */
		 ...
		 */
		[[nodiscard]] virtual IoResult<void_t> SubmitSendRegistered(socket_t _socket, BufferHandle _handle, const void_t* _buffer, std::size_t _length, void_t* _userData) noexcept override { return IoResult<void_t>::Error(IoError{ IoErrorKind::UNSUPPORTED, "io_uring fixed buffers are consumed via ReadFixed/WriteFixed, not this method" }); }
		/** @brief 항상 UNSUPPORTED 를 반환한다(SubmitSendRegistered 와 동일한 이유). */
		[[nodiscard]] virtual IoResult<void_t> SubmitReceiveRegistered(socket_t _socket, BufferHandle _handle, void_t* _buffer, std::size_t _length, void_t* _userData) noexcept override { return IoResult<void_t>::Error(IoError{ IoErrorKind::UNSUPPORTED, "io_uring fixed buffers are consumed via ReadFixed/WriteFixed, not this method" }); }
