...
// ─── 팩토리 ─────────────────────────────────────────────────────────────────
		...
		const ne::string_t& _path, FileIocpEngine& _engine, const bool_t _readOnly) noexcept
	{
		...
		const HANDLE handle = ::CreateFileA(
			_path.c_str(), access, FILE_SHARE_READ, nullptr,
...
#elif defined(IS_POSIX)
	ne::Result<AsyncFile, ne::OsError> AsyncFile::Create(
		const ne::string_t& _path, IoUringEngine& _engine) noexcept
	{
	...
	ne::Result<AsyncFile, ne::OsError> AsyncFile::Open(
		const ne::string_t& _path, IoUringEngine& _engine, const bool_t _readOnly) noexcept
	{
	...
	ne::Result<AsyncFile, ne::OsError> AsyncFile::Open(
		const ne::string_t& _path, const bool_t _readOnly) noexcept
	{
		(void)_path;
		(void)_readOnly;
		return ne::Result<AsyncFile, ne::OsError>::Error(
...
// ─── 비동기 Read / Write ────────────────────────────────────────────────────
	...
	ne::Task<ne::Result<std::size_t, ne::OsError>> AsyncFile::Read(
		std::span<ne::byte_t> _buf, const std::size_t _offset)
	{
		if (!IsOpen()) co_return ne::Result<std::size_t, ne::OsError>::Error(ne::OsError{ 0, "file not open" });

#if defined(IS_POSIX) || defined(_WIN32)
		co_return co_await FileReadAwaitable{ *engine, fd, _buf, _offset };
#else
		co_return ne::Result<std::size_t, ne::OsError>::Error(ne::OsError{ 0, "not supported" });
#endif
	}

	ne::Task<ne::Result<std::size_t, ne::OsError>> AsyncFile::Write(
		std::span<const ne::byte_t> _data, const std::size_t _offset)
	{
		if (!IsOpen()) co_return ne::Result<std::size_t, ne::OsError>::Error(ne::OsError{ 0, "file not open" });
