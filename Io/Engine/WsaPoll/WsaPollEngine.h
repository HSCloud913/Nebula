...
// IIoEngine 계약(Submit/WaitCompletions)으로 노출한다. IocpEngine 생성이 실패하는 극히
...
BEGIN_NS(ne::io)
	class WsaPollEngine final :public IIoEngine
	{
	public:
	...
	private:
		ulonglong_t wakeReadSocket{ 0 };  // socket_t 를 64비트로 정규화(IoRequest.handle 과 동일 관례)
		ulonglong_t wakeWriteSocket{ 0 };
		bool_t isValid{ false };
		std::mutex mutex;
		std::unordered_map<void*, PendingOperation> pending;    // userData → 대기 op
		std::unordered_map<ulonglong_t, void*> readWaiter;  // fd → POLLIN 대기 userData
		std::unordered_map<ulonglong_t, void*> writeWaiter; // fd → POLLOUT 대기 userData
