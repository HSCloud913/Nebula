...
// IoUringEngine::Submit() 의 OpCode::ReadFixed/WriteFixed 처리에서 IoRequest.bufferId 로 하고
...
#pragma once
#include "Type.h" // IS_POSIX 정의 — 아래 가드 전에 반드시 포함
#if defined(IS_POSIX)

#include "Engine/IRegisteredBufferProvider.h"
#include <liburing.h>
#include <mutex>
#include <vector>
...
BEGIN_NS(ne::io)
	...
	public:
		...
		[[nodiscard]] virtual ne::Result<void, ne::OsError> SubmitSendRegistered(socket_t _socket, BufferHandle _handle, const void* _buffer, std::size_t _length, void* _userData) noexcept override;
		[[nodiscard]] virtual ne::Result<void, ne::OsError> SubmitReceiveRegistered(socket_t _socket, BufferHandle _handle, void* _buffer, std::size_t _length, void* _userData) noexcept override;
