...
//
// Level 0 — Linux io_uring 백엔드. native 완료 큐를 IEngine 계약에 그대로 매핑한다(스펙 2.1).
//   Submit          : op 별 SQE 준비(prep_read/write/recv/send/accept/connect) + user_data=op
//   WaitCompletions : wait_cqe_timeout → peek_batch_cqe → Completion 으로 정규화(cqe->res)
...
BEGIN_NS(ne::io)
	...
	private:
		...
		std::unordered_map<void_t*, UringOperation*> inflight; // handler(userData) → op (Cancel 조회)
		std::vector<void_t*> pendingCancels;                   // 타 스레드 Cancel 요청 — 루프 스레드가 ASYNC_CANCEL 제출
