...
BEGIN_NS(ne::io)
	...
	ne::Result<void_t, IoError> RioProvider::EnsureInitialized() noexcept
	{
		...
		const SOCKET tmp = ::WSASocketW(AF_INET, SOCK_STREAM, IPPROTO_TCP, nullptr, 0, WSA_FLAG_OVERLAPPED | WSA_FLAG_REGISTERED_IO);
		if (tmp == INVALID_SOCKET) return ne::Result<void_t, IoError>::Error(IoError{ ne::OsError{ ne::LastOsError() } }.Context("[RioProvider/WSASocketW]"));

		GUID guid = WSAID_MULTIPLE_RIO;
		ulong_t bytes = 0;
		table.cbSize = sizeof(table);
		const int rc = ::WSAIoctl(tmp, SIO_GET_MULTIPLE_EXTENSION_FUNCTION_POINTERS, &guid, sizeof(guid), &table, sizeof(table), &bytes, nullptr, nullptr);
		::closesocket(tmp);

		if (rc == SOCKET_ERROR || !table.RIORegisterBuffer || !table.RIOCreateCompletionQueue)
			return ne::Result<void_t, IoError>::Error(IoError{ ne::OsError{ ne::LastOsError() } }.Context("[RioProvider/WSAIoctl(RIO)]"));
	...
	ne::Result<void_t, IoError> RioProvider::Submit(const socket_t _socket, const BufferHandle _handle, const void_t* _buffer, const std::size_t _length, void_t* _userData,
		const bool_t _isSend) noexcept
	{
		...
		const BOOL ok = _isSend ? table.RIOSend(rq.Value(), &localBuf, 1, 0, reinterpret_cast<PVOID>(_userData)) : table.RIOReceive(rq.Value(), &localBuf, 1, 0, reinterpret_cast<PVOID>(_userData));
		if (!ok) return ne::Result<void_t, IoError>::Error(IoError{ ne::OsError{ ne::LastOsError() } }.Context(_isSend ? "[RioProvider/RIOSend]" : "[RioProvider/RIOReceive]"));
	...
	ne::Result<void_t, IoError> RioProvider::ArmNotify() noexcept
	{
		...
		if (const int rc = table.RIONotify(cq); rc != 0) // 0 = ERROR_SUCCESS
			return ne::Result<void_t, IoError>::Error(IoError{ ne::OsError{ static_cast<ne::ulong_t>(rc) } }.Context("[RioProvider/RIONotify]"));
