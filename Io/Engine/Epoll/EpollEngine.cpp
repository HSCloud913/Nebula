...
BEGIN_NS(ne::io)
	EpollEngine::EpollEngine() noexcept
	{
		...
		if (wakeEventFd < 0) { ::close(epollFd); epollFd = -1; return; }
		...
		{
			::close(wakeEventFd); ::close(epollFd); wakeEventFd = -1; epollFd = -1; return;
		}
	...
	bool_t EpollEngine::Perform(const IoRequest& _request, const bool_t _isRetry, longlong_t& _result) noexcept
	{
		...
		case OpCode::Accept:
		{
			...
			if (accepted >= 0) { _result = static_cast<longlong_t>(accepted); return true; }
			break;
		}
		case OpCode::Connect:
		{
			...
			{
				_result = 0; return true; // 즉시 연결(로컬 등)
			}
	...
	void_t EpollEngine::UpdateEpoll(const int_t _fd) noexcept
	{
		...
		event.events = events;
		event.data.fd = _fd;
		if (::epoll_ctl(epollFd, EPOLL_CTL_MOD, _fd, &event) != 0)
			(void)::epoll_ctl(epollFd, EPOLL_CTL_ADD, _fd, &event); // 미등록이면 ADD
	}

	void_t EpollEngine::Submit(const IoRequest& _request)
	{
		longlong_t result = 0;
		if (Perform(_request, false, result))
		{
		...
		if (isWrite) writeWaiter[fd] = _request.userData; else readWaiter[fd] = _request.userData;
		UpdateEpoll(fd);
	}
	...
	void_t EpollEngine::ProcessCancels()
	{
		...
		for (void* userData : cancels)
		{
			...
			if (iterator->second.isWrite) writeWaiter.erase(fd); else readWaiter.erase(fd);
		...
		for (int_t i = 0; i < eventCount && count < _max; ++i)
		{
			...
			for (int_t direction = 0; direction < 2 && count < _max; ++direction)
			{
				...
				bool_t found = false;
				{
					...
					if (pendingIterator == pending.end()) { waiter.erase(iterator); continue; }
	...
	bool_t EpollEngine::Supports(const Capability _capability) const noexcept
	{
		switch (_capability)
		{
		case Capability::SendFileZeroCopy:    return true;  // sendfile / splice
		case Capability::SendMemZeroCopy:     return false;
		case Capability::RecvOverheadReduced: return false;
