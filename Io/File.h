...
#if defined(_WIN32)
#	include "Base/WindowsApi.h" // FileHandle 의 deleter 가 ::CloseHandle 을 직접 호출
#endif
...
namespace ne::io
{
	 ...
	 * (span/ne::memory::BufferChain)를 그대로 커널에 넘기므로 완료될 때까지 호출자가 File 과 버퍼의
	 * 수명을 보장해야 한다.
	 */
	class File
	{
	...
	public:
		...
		[[nodiscard]] ne::Task<IoResult<std::size_t>> Readv(const ne::memory::BufferChain& _chain, ulonglong_t _offset, std::stop_token _stopToken = {});
		[[nodiscard]] ne::Task<IoResult<std::size_t>> Writev(const ne::memory::BufferChain& _chain, ulonglong_t _offset, std::stop_token _stopToken = {});
