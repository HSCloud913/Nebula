#include "Cryptography/AES.h"

#include <array>
#include <string>
#include <utility>
#include "Cryptography/Internal/Ecb.h"
#include "Util/SecureRandom.h"
#include "Util/SecureWipe.h"



static constexpr ne::byte_t Sbox[256] = {  };

static constexpr ne::byte_t InvSbox[256] = {  };

// Rcon(라운드 상수): GF(2^8)에서 x^i를 기약다항식 0x11b로 축소한 값들.
// 키 확장 시 매 keyWordLength번째 워드에 XOR되어, 동일한 키 워드에서 라운드마다 서로 다른 값이 나오게 한다(대칭성 제거).
static constexpr ne::byte_t Rcon[11] = { 0x00, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36 };

struct AESContext
{
	ne::byte_t roundKey[240];
	int round;
};

using Block = std::array<ne::byte_t, 16>;



// xtime: GF(2^8)에서 x2(즉 곱하기 2) 연산. 왼쪽으로 1비트 시프트한 뒤, 최상위 비트가 넘쳤다면(오버플로)
// AES의 기약다항식 x^8+x^4+x^3+x+1(=0x11b)로 모듈러 축소하기 위해 0x1b를 XOR한다(최상위 비트는 이미 시프트로 잘려나감).
static ne::byte_t xtime(const ne::byte_t _a) { return static_cast<ne::byte_t>((_a << 1) ^ ((_a & 0x80) ? 0x1b : 0x00)); }

// gmul: GF(2^8) 유한체에서의 곱셈(러시안 농부 곱셈). b의 각 비트에 대해 필요하면 현재 a를 결과에 XOR하고,
// a는 xtime으로 2를 곱해가며 다음 자리로 진행한다(일반 정수 곱셈의 "덧셈 대신 XOR, 자리올림 대신 기약다항식 축소" 버전).
static ne::byte_t gmul(ne::byte_t _a, ne::byte_t _b)
{
	ne::byte_t p = 0;
	while (_b)
	{
		if (_b & 1) p ^= _a;
		_a = xtime(_a);
		_b >>= 1;
	}

	return p;
}

static void KeyExpansion(AESContext& _context, const ne::byte_t* _key, const int _keyWordLength)
{
	const int totalWords = 4 * (_context.round + 1);
	ne::byte_t* w = _context.roundKey;

	for (int i = 0; i < _keyWordLength; ++i)
	{
		w[i * 4 + 0] = _key[i * 4 + 0];
		w[i * 4 + 1] = _key[i * 4 + 1];
		w[i * 4 + 2] = _key[i * 4 + 2];
		w[i * 4 + 3] = _key[i * 4 + 3];
	}

	for (int i = _keyWordLength; i < totalWords; ++i)
	{
		ne::byte_t temp[4];
		temp[0] = w[(i - 1) * 4 + 0];
		temp[1] = w[(i - 1) * 4 + 1];
		temp[2] = w[(i - 1) * 4 + 2];
		temp[3] = w[(i - 1) * 4 + 3];

		// 키 길이(워드 단위) 경계에 도달할 때마다 RotWord(1바이트 순환), SubWord(S-box 치환), Rcon XOR을 적용해
		// 확장된 키 워드가 원본 키와 선형적으로 유사해지지 않도록 매 라운드 다른 값을 주입한다.
		if (i % _keyWordLength == 0)
		{
			const ne::byte_t t = temp[0];
			temp[0] = Sbox[temp[1]] ^ Rcon[i / _keyWordLength];
			temp[1] = Sbox[temp[2]];
			temp[2] = Sbox[temp[3]];
			temp[3] = Sbox[t];
		}
		// AES-256(keyWordLength=8) 한정 추가 규칙: 8워드 주기의 중간(오프셋 4)에서도 SubWord만 한 번 더 적용해
		// 확산을 보강한다(키가 길어 라운드 수 대비 확장 주기가 길어지는 것을 보완).
		else if (_keyWordLength > 6 && i % _keyWordLength == 4)
		{
			temp[0] = Sbox[temp[0]];
			temp[1] = Sbox[temp[1]];
			temp[2] = Sbox[temp[2]];
			temp[3] = Sbox[temp[3]];
		}

		w[i * 4 + 0] = w[(i - _keyWordLength) * 4 + 0] ^ temp[0];
		w[i * 4 + 1] = w[(i - _keyWordLength) * 4 + 1] ^ temp[1];
		w[i * 4 + 2] = w[(i - _keyWordLength) * 4 + 2] ^ temp[2];
		w[i * 4 + 3] = w[(i - _keyWordLength) * 4 + 3] ^ temp[3];
	}
}

static void AddRoundKey(Block& _state, const ne::byte_t* _roundKey, int _round)
{
	for (int c = 0; c < 4; ++c)
	{
		for (int r = 0; r < 4; ++r)
		{
			_state[r + 4 * c] ^= _roundKey[(_round * 4 + c) * 4 + r];
		}
	}
}

static void SubBytes(Block& _state) { for (auto& b : _state) b = Sbox[b]; }

static void InvSubBytes(Block& _state) { for (auto& b : _state) b = InvSbox[b]; }

static void ShiftRows(Block& _state)
{
	ne::byte_t t = _state[1];
	_state[1] = _state[5];
	_state[5] = _state[9];
	_state[9] = _state[13];
	_state[13] = t;

	std::swap(_state[2], _state[10]);
	std::swap(_state[6], _state[14]);

	t = _state[15];
	_state[15] = _state[11];
	_state[11] = _state[7];
	_state[7] = _state[3];
	_state[3] = t;
}

static void InvShiftRows(Block& _state)
{
	ne::byte_t t = _state[13];
	_state[13] = _state[9];
	_state[9] = _state[5];
	_state[5] = _state[1];
	_state[1] = t;

	std::swap(_state[2], _state[10]);
	std::swap(_state[6], _state[14]);

	t = _state[3];
	_state[3] = _state[7];
	_state[7] = _state[11];
	_state[11] = _state[15];
	_state[15] = t;
}

// 각 열을 고정 행렬 [02 03 01 01; 01 02 03 01; 01 01 02 03; 03 01 01 02]과 GF(2^8) 상에서 곱하는 연산.
// 이 행렬은 MDS(Maximum Distance Separable) 성질을 가져 바이트 간 확산(diffusion)을 최대화한다.
static void MixColumns(Block& _state)
{
	for (int c = 0; c < 4; ++c)
	{
		ne::byte_t s0 = _state[4 * c + 0], s1 = _state[4 * c + 1];
		ne::byte_t s2 = _state[4 * c + 2], s3 = _state[4 * c + 3];

		_state[4 * c + 0] = gmul(0x02, s0) ^ gmul(0x03, s1) ^ s2 ^ s3;
		_state[4 * c + 1] = s0 ^ gmul(0x02, s1) ^ gmul(0x03, s2) ^ s3;
		_state[4 * c + 2] = s0 ^ s1 ^ gmul(0x02, s2) ^ gmul(0x03, s3);
		_state[4 * c + 3] = gmul(0x03, s0) ^ s1 ^ s2 ^ gmul(0x02, s3);
	}
}

// MixColumns의 역행렬 [0e 0b 0d 09; ...]을 곱한다. 이 계수들은 GF(2^8)에서 MixColumns 행렬의 역원이므로
// InvMixColumns(MixColumns(x)) == x가 성립한다.
static void InvMixColumns(Block& _state)
{
	for (int c = 0; c < 4; ++c)
	{
		ne::byte_t s0 = _state[4 * c + 0], s1 = _state[4 * c + 1];
		ne::byte_t s2 = _state[4 * c + 2], s3 = _state[4 * c + 3];

		_state[4 * c + 0] = gmul(0x0e, s0) ^ gmul(0x0b, s1) ^ gmul(0x0d, s2) ^ gmul(0x09, s3);
		_state[4 * c + 1] = gmul(0x09, s0) ^ gmul(0x0e, s1) ^ gmul(0x0b, s2) ^ gmul(0x0d, s3);
		_state[4 * c + 2] = gmul(0x0d, s0) ^ gmul(0x09, s1) ^ gmul(0x0e, s2) ^ gmul(0x0b, s3);
		_state[4 * c + 3] = gmul(0x0b, s0) ^ gmul(0x0d, s1) ^ gmul(0x09, s2) ^ gmul(0x0e, s3);
	}
}

static Block EncryptBlock(const AESContext& _context, Block _state)
{
	AddRoundKey(_state, _context.roundKey, 0);

	for (int round = 1; round < _context.round; ++round)
	{
		SubBytes(_state);
		ShiftRows(_state);
		MixColumns(_state);
		AddRoundKey(_state, _context.roundKey, round);
	}

	SubBytes(_state);
	ShiftRows(_state);
	AddRoundKey(_state, _context.roundKey, _context.round);

	return _state;
}

static Block DecryptBlock(const AESContext& _context, Block _state)
{
	AddRoundKey(_state, _context.roundKey, _context.round);

	for (int round = _context.round - 1; round >= 1; --round)
	{
		InvShiftRows(_state);
		InvSubBytes(_state);
		AddRoundKey(_state, _context.roundKey, round);
		InvMixColumns(_state);
	}

	InvShiftRows(_state);
	InvSubBytes(_state);
	AddRoundKey(_state, _context.roundKey, 0);

	return _state;
}

static AESContext MakeContext(const ne::crypto::AES::Type _keyType, const ne::string_t& _key)
{
	AESContext context{};
	int keyWordLength;

	switch (_keyType)
	{
		case ne::crypto::AES::Type::AES_128:
			keyWordLength = 4;
			context.round = 10;
			break;
		case ne::crypto::AES::Type::AES_192:
			keyWordLength = 6;
			context.round = 12;
			break;
		default:
			keyWordLength = 8;
			context.round = 14;
			break;
	}

	KeyExpansion(context, reinterpret_cast<const ne::byte_t*>(_key.data()), keyWordLength);

	return context;
}

static Block ToBlock(const ne::string_view_t _data, const size_t _offset)
{
	Block b{};
	for (int i = 0; i < 16; ++i) b[i] = static_cast<ne::byte_t>(_data[_offset + i]);

	return b;
}

static void ApplyPKCS7(ne::string_t& _data)
{
	const auto pad = static_cast<ne::byte_t>(16 - (_data.size() % 16));
	_data.append(pad, static_cast<ne::char_t>(pad));
}

// 패딩이 깨졌으면 실패를 알린다 — 과거에는 입력을 그대로 돌려줘서 잘못된 키/변조된 암호문이
// 성공처럼 보였다(조용한 데이터 오염). 이 실패 사유를 원격에 노출하면 패딩 오라클이 되므로
// 인증이 필요한 경로는 Open() 을 써야 한다.
static ne::crypto::CryptoResult<ne::string_t> RemovePKCS7(ne::string_t _data)
{
	using R = ne::crypto::CryptoResult<ne::string_t>;
	using ne::crypto::CryptoError;
	using ne::crypto::CryptoErrorKind;

	if (_data.empty() || _data.size() % 16 != 0) return R::Error(CryptoError{ CryptoErrorKind::INVALID_INPUT, "ciphertext is not block aligned" });

	const auto pad = static_cast<ne::byte_t>(_data.back());
	if (pad == 0 || pad > 16) return R::Error(CryptoError{ CryptoErrorKind::MALFORMED_PADDING });

	for (size_t i = _data.size() - pad; i < _data.size(); ++i)
	{
		if (static_cast<ne::byte_t>(_data[i]) != pad) return R::Error(CryptoError{ CryptoErrorKind::MALFORMED_PADDING });
	}

	_data.resize(_data.size() - pad);

	return R::Ok(std::move(_data));
}

// CBC 암호화 코어 — 공개 오버로드 두 개(명시 IV / 자동 IV)가 공유한다.
static ne::string_t EncryptCbcCore(const AESContext& _context, const ne::string_view_t _iv, const ne::string_view_t _plaintext)
{
	ne::string_t padded(_plaintext);
	ApplyPKCS7(padded);

	ne::string_t result;
	result.reserve(padded.size());

	Block prev = ToBlock(_iv, 0);
	for (size_t i = 0; i < padded.size(); i += 16)
	{
		Block block = ToBlock(padded, i);
		for (ne::int_t j = 0; j < 16; ++j) block[j] ^= prev[j];

		prev = EncryptBlock(_context, block);
		for (const auto data : prev) result += static_cast<ne::char_t>(data);
	}

	return result;
}



namespace ne::crypto
{
	AES::AES(const Type _type, string_t _key) noexcept
		: type(_type)
		, key(std::move(_key)) {}

	AES::~AES() { ne::util::SecureWipe(key); }



	CryptoResult<AES> AES::Create(const Type _type, const string_view_t _key)
	{
		using R = CryptoResult<AES>;

		const std::size_t required = KeyLength(_type);
		if (_key.size() != required) return R::Error(CryptoError{ CryptoErrorKind::INVALID_KEY_LENGTH, "AES key must be " + std::to_string(required) + " bytes" });

		return R::Ok(AES{ _type, string_t(_key) });
	}



	CryptoResult<string_t> AES::EncryptCBC(const string_view_t _iv, const string_view_t _plaintext) const
	{
		using R = CryptoResult<string_t>;

		if (_iv.size() != BlockSize) return R::Error(CryptoError{ CryptoErrorKind::INVALID_INPUT, "CBC IV must be 16 bytes" });

		return R::Ok(EncryptCbcCore(MakeContext(type, key), _iv, _plaintext));
	}

	CryptoResult<AES::CbcCiphertext> AES::EncryptCBC(const string_view_t _plaintext) const
	{
		using R = CryptoResult<CbcCiphertext>;

		string_t iv(BlockSize, '\0');
		ne::util::SecureRandom random;
		if (!random.Fill(iv.data(), iv.size())) return R::Error(CryptoError{ CryptoErrorKind::RANDOM_FAILURE, "failed to generate CBC IV" });

		string_t ciphertext = EncryptCbcCore(MakeContext(type, key), iv, _plaintext);

		return R::Ok(CbcCiphertext{ std::move(iv), std::move(ciphertext) });
	}

	CryptoResult<string_t> AES::DecryptCBC(const string_view_t _iv, const string_view_t _ciphertext) const
	{
		using R = CryptoResult<string_t>;

		if (_iv.size() != BlockSize) return R::Error(CryptoError{ CryptoErrorKind::INVALID_INPUT, "CBC IV must be 16 bytes" });
		if (_ciphertext.empty() || _ciphertext.size() % BlockSize != 0) return R::Error(CryptoError{ CryptoErrorKind::INVALID_INPUT, "ciphertext is not block aligned" });

		const AESContext context = MakeContext(type, key);

		string_t result;
		result.reserve(_ciphertext.size());

		Block prev = ToBlock(_iv, 0);
		for (size_t i = 0; i < _ciphertext.size(); i += BlockSize)
		{
			const Block cipher = ToBlock(_ciphertext, i);

			Block plain = DecryptBlock(context, cipher);
			for (int_t j = 0; j < 16; ++j) plain[j] ^= prev[j];

			prev = cipher;
			for (const auto data : plain) result += static_cast<char_t>(data);
		}

		return RemovePKCS7(std::move(result));
	}



	CryptoResult<AES::CbcCiphertext> AesEncrypt(const AES::Type _type, const string_view_t _key, const string_view_t _plaintext)
	{
		auto cipher = AES::Create(_type, _key);
		if (cipher.IsError()) return CryptoResult<AES::CbcCiphertext>::Error(std::move(cipher.Error()));

		return cipher.Value().EncryptCBC(_plaintext);
	}

	CryptoResult<string_t> AesDecrypt(const AES::Type _type, const string_view_t _key, const string_view_t _iv, const string_view_t _ciphertext)
	{
		auto cipher = AES::Create(_type, _key);
		if (cipher.IsError()) return CryptoResult<string_t>::Error(std::move(cipher.Error()));

		return cipher.Value().DecryptCBC(_iv, _ciphertext);
	}
}



namespace ne::crypto::internal
{
	CryptoResult<string_t> EncryptEcb(const AES::Type _type, const string_view_t _key, const string_view_t _plaintext)
	{
		using R = CryptoResult<string_t>;

		if (_key.size() != AES::KeyLength(_type)) return R::Error(CryptoError{ CryptoErrorKind::INVALID_KEY_LENGTH });

		string_t padded(_plaintext);
		ApplyPKCS7(padded);

		const AESContext context = MakeContext(_type, string_t(_key));

		string_t result;
		result.reserve(padded.size());

		for (size_t i = 0; i < padded.size(); i += AES::BlockSize)
		{
			for (const auto data : EncryptBlock(context, ToBlock(padded, i))) result += static_cast<char_t>(data);
		}

		return R::Ok(std::move(result));
	}

	CryptoResult<string_t> DecryptEcb(const AES::Type _type, const string_view_t _key, const string_view_t _ciphertext)
	{
		using R = CryptoResult<string_t>;

		if (_key.size() != AES::KeyLength(_type)) return R::Error(CryptoError{ CryptoErrorKind::INVALID_KEY_LENGTH });
		if (_ciphertext.empty() || _ciphertext.size() % AES::BlockSize != 0) return R::Error(CryptoError{ CryptoErrorKind::INVALID_INPUT, "ciphertext is not block aligned" });

		const AESContext context = MakeContext(_type, string_t(_key));

		string_t result;
		result.reserve(_ciphertext.size());

		for (size_t i = 0; i < _ciphertext.size(); i += AES::BlockSize)
		{
			for (const auto data : DecryptBlock(context, ToBlock(_ciphertext, i))) result += static_cast<char_t>(data);
		}

		return RemovePKCS7(std::move(result));
	}
}
