...
BEGIN_NS(ne::crypto)
	/**
	 * @class SecureRandom
	 * @brief 암호학적 난수원(CSPRNG) 래퍼입니다. mt19937 등 비암호 PRNG 대신 키/소수/패딩 생성에 사용합니다.
	 *
	 * Windows에서는 BCryptGenRandom(BCRYPT_USE_SYSTEM_PREFERRED_RNG, WinCNG 백엔드), POSIX에서는
	 * getrandom(2)(실패 시 /dev/urandom 폴백)를 사용합니다. UniformRandomBitGenerator를
	 * 모델링하므로 std::uniform_int_distribution 등과 그대로 조합해 쓸 수 있습니다.
	 */
	class SecureRandom
	{
	public:
